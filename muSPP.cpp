...
int main() {
    ...
    auto *n1 = solver.addNode(); // id = 2
    auto *n2 = solver.addNode(); // id = 3
    auto *n3 = solver.addNode(); // id = 4

    // Add edges with different capacities and costs
    solver.addEdge(solver.getSource(), n1, 0, 3); // s -> 2
    solver.addEdge(solver.getSource(), n2, 0, 2); // s -> 3
    solver.addEdge(n1, n2, -2, 2);                // 2 -> 3
    solver.addEdge(n1, n3, -3, 2);                // 2 -> 4
    solver.addEdge(n2, n3, -1, 2);                // 3 -> 4
    solver.addEdge(n3, solver.getSink(), 0, 4);   // 4 -> t
