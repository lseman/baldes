...
void Sink::sink_update_all(std::vector<SSPNode> &V, std::vector<double> &distance2src, int sink_id, int n){
    double sink_dist;
    for(int i=2; i<n; i+=2){
...
void Sink::sink_update_all_weight(std::vector<SSPNode> &V, std::vector<double> &distance2src, int sink_id, int n){
    double sink_dist;
    for(int i=2; i<n; i+=2){
        sink_dist = V[i].successor_edges_weights[0] - distance2src[sink_id];
        sink_precursor_weights[i] = sink_dist + distance2src[i];
