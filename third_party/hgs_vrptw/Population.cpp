...
void Population::doHGSLocalSearchAndAddIndividual(Individual *indiv) {
    ...
    if (!indiv->isFeasible &&
        ...
        localSearch->run(indiv, params->penaltyCapacity * 10.,
...
void Population::generatePopulation() {
    // Special-case: Only one client—generate the single solution quickly.
    ...
    // If an initial solution is provided, add it first.
    if (!params->config.initialSolution.empty()) {
    ...
    const double fractionNearest = params->config.fractionGeneratedNearest;
    const double fractionFurthest = params->config.fractionGeneratedFurthest;
    const double fractionSweep = params->config.fractionGeneratedSweep;
    const double fractionRandom = params->config.fractionGeneratedRandomly;
    const double fractionMDM = params->config.fractionGeneratedMDM;
    int minSweepFillPercentage =
        std::max(params->config.minSweepFillPercentage, 30);
    ...
    const double initialTimeWarpPenalty = params->config.initialTimeWarpPenalty;

    // Total number of individuals to generate.
    int totalIndividuals = 4 * params->config.minimumPopulationSize;

    // Reset the penalty (note: this also resets when the population is reset).
    params->penaltyTimeWarp = initialTimeWarpPenalty;

    // Determine counts for each construction heuristic.
    int countNearest = round(fractionNearest * totalIndividuals);
    int countFurthest = round(fractionFurthest * totalIndividuals);
    int countSweep = round(fractionSweep * totalIndividuals);
    int countRandom = round(fractionRandom * totalIndividuals);
    int countMDM = round(fractionMDM * totalIndividuals);

    // --- Generate individuals using MDMPatterns ---
    for (int i = 0; i < countMDM; i++) {
    ...
    // --- Generate individuals using the NEAREST construction heuristic ---
    for (int i = 0; i < countNearest; i++) {
    ...
    // --- Generate individuals using the FURTHEST construction heuristic ---
    for (int i = 0; i < countFurthest; i++) {
        if (params->isTimeLimitExceeded()) {
            std::cout << "Time limit during generation of initial population"
                      << std::endl;
            printState(-1, -1);
    ...
    // --- Generate individuals using the SWEEP construction heuristic ---
    for (int i = 0; i < countSweep; i++) {
        if (params->isTimeLimitExceeded()) {
            std::cout << "Time limit during generation of initial population"
                      << std::endl;
            printState(-1, -1);
        ...
        // For the first individual use 100% fill; thereafter, choose a random
        // fill percentage.
        int fillPercentage =
            (i == 0) ? 100
    ...
    // --- Generate individuals using a RANDOM strategy ---
...
bool Population::addIndividual(const Individual *indiv, bool updateFeasible) {
    ...
    while (place > 0 && subpop[place - 1]->myCostSol.penalizedCost >
    ...
    if (indiv->isFeasible &&
        ...
        searchProgress.push_back({params->getTimeElapsedSeconds(),
    ...
    if (pop.size() == 1) {
        ...
        for (int i = 0; i < static_cast<int>(pop.size()); i++) {
            ...
            if (static_cast<int>(pop.size()) <= params->config.nbElite) {
                ...
                pop[ranking[i].second]->biasedFitness =
                    fitRank +
                    (1.0 - static_cast<double>(params->config.nbElite) /
...
Individual *Population::getBinaryTournament() {
    ...
    int place1 = params->rng() % (feasibleSubpopulation.size() +
    ...
    // feasible and infeasible individuals)
    int place2 = params->rng() % (feasibleSubpopulation.size() +
                                  infeasibleSubpopulation.size());
...
void Population::printState(int nbIter, int nbIterNoImprovement) {
    ...
    std::printf("| It %6d %6d | T(s) %.2f", nbIter, nbIterNoImprovement,
    ...
    if (getBestFeasible() != nullptr) {
        std::printf(" | Feas %zu %.2f %.2f", feasibleSubpopulation.size(),
    ...
    std::printf(" | Div %.2f %.2f", getDiversity(feasibleSubpopulation),
    ...
    std::printf(
        ...
        static_cast<double>(std::count(listFeasibilityTimeWarp.begin(),
                                       listFeasibilityTimeWarp.end(), true)) /
            static_cast<double>(listFeasibilityTimeWarp.size()));
    std::printf(" | Pen %.2f %.2f |", params->penaltyCapacity,
                params->penaltyTimeWarp);
    std::cout << std::endl;
}
...
Population::Population(Params *params, Split *split,
...
// (2) the MDM elite set is not full OR this individual has a better penalized
// cost than at least one of those already in the set
void Population::updateMDMElite(const Individual *indiv) {
    ...
    // If the number of elites exceeds the limit, remove the worst individual
    // (which is on top of the priority queue)
...
void Population::mineElite() {
    ...
    if (mdmEliteUpdated &&
        mdmEliteNonUpdatingRestarts >= mdmEliteMaxNonUpdatingRestarts &&
        mdmElite.size() > 1) {
        if (params->verbose) {
            std::cout << "----- MINING PATTERNS FROM MDM ELITE SET -----"
                      << std::endl;
        }
        ...
        for (const auto &indiv : eliteVector) {
            ...
            for (int r = 0; r < params->nbVehicles; ++r) {
                ...
                for (int c = 0; c < static_cast<int>(chromR.size()) - 1; ++c) {
                    int index = chromR[c] * nbNodes +
                                chromR[c + 1];  // Map 2D matrix to vector index
