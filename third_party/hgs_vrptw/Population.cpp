...
void Population::exportBKS(std::string fileName) {
    ...
    if (bestSolutionOverall.myCostSol.penalizedCost < 1.e29 &&
        (!readOK || bestSolutionOverall.myCostSol.penalizedCost < readCost - MY_EPSILON)) {
...
void Population::exportSearchProgress(std::string fileName, std::string instanceName, int seedRNG) {
    ...
    for (std::pair<clock_t, double> state : searchProgress) {
        myfile << instanceName << ";" << seedRNG << ";" << state.second << ";"
               << static_cast<double>(state.first) / CLOCKS_PER_SEC << std::endl;
    }
}
...
void Population::exportPopulation(int nbIter, std::string fileName) {
    ...
    if (myfile.is_open()) {
        ...
        for (Individual *indiv : feasibleSubpopulation) { logSolution(nbIter, myfile, indiv); }

        // Log all the infeasible solutions
        for (Individual *indiv : infeasibleSubpopulation) { logSolution(nbIter, myfile, indiv); }
    } else {
...
Population::Population(Params *params, Split *split, HGSLocalSearch *localSearch)
    : params(params), split(split), localSearch(localSearch), mdmElite(compareIndiv) {
...
void Population::updateMDMElite(const Individual *indiv) {
    ...
    if (mdmElite.size() > mdmNbElite) // If max number of elite individuals is exceeded, remove the worst
    {
        // std::set<Individual>::iterator it = --mdmElite.end();
        // std::set<const Individual *>::iterator *it = --mdmElite.end();
        std::set<const Individual *>::iterator it = --mdmElite.end();
