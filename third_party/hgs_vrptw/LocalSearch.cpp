...
bool operator==(const TimeWindowData &twData1, const TimeWindowData &twData2) {
    return twData1.firstNodeIndex == twData2.firstNodeIndex && twData1.lastNodeIndex == twData2.lastNodeIndex &&
           ...
           twData1.earliestArrival == twData2.earliestArrival && twData1.latestArrival == twData2.latestArrival;
}
...
void HGSLocalSearch::constructIndividualBySweep(int fillPercentage, Individual *indiv) {
    ...
    for (int i = 0; i < static_cast<int>(nodesToInsert.size()); i++) {
        if (load > 0 && load + nodesToInsert[i].load > fillPercentage * params->vehicleCapacity / 100 &&
    ...
    for (int r = 0; r < static_cast<int>(nodeIndicesPerRoute.size()); r++) {
        ...
        for (int idx : nodeIndicesPerRoute[r]) {
            ...
            if ((nodesToInsert[idx].twData.latestArrival - nodesToInsert[idx].twData.earliestArrival) * 2 >
        ...
        for (int i = 0; i < static_cast<int>(nodeIndicesToInsertLongTw.size()); i++) {
            ...
            for (int j = 0; j <= routes[r].nbCustomers; j++) {
                ...
                double insertionCost =
                    params->timeCost.get(prev->cour, nodesToInsert[nodeIndicesToInsertLongTw[i]].clientIdx) +
...
void HGSLocalSearch::constructIndividualWithSeedOrder(int toleratedCapacityViolation, int toleratedTimeWarp,
                                                      bool useSeedClientFurthestFromDepot, Individual *indiv) {
    std::vector<NodeToInsert> nodesToInsert;
    initializeConstruction(indiv, &nodesToInsert);

    std::set<int> unassignedNodeIndices;
    for (int i = 1; i <= params->nbClients; i++) { unassignedNodeIndices.insert(i - 1); }
...
void HGSLocalSearch::run(Individual *indiv, double penaltyCapacityLS, double penaltyTimeWarpLS) {
    ...
    for (int i = 1; i <= params->nbClients; i++)
        if (params->rng() % params->config.nbGranular ==
            0) // Designed to use O(nbGranular x n) time overall to avoid possible bottlenecks
    ...
    for (loopID = 0; !searchCompleted; loopID++) {
        ...
        for (int posU = 0; posU < params->nbClients; posU++) {
            ...
            /* MOVES INVOLVING AN EMPTY ROUTE -- NOT TESTED IN THE FIRST LOOP TO AVOID INCREASING TOO MUCH THE FLEET
             * SIZE */
        ...
        if (!neverIntensify && searchCompleted && (alwaysIntensify || runLS_INT)) {
            for (int rU = 0; rU < params->nbVehicles; rU++) {
                ...
                for (int rV = 0; rV < params->nbVehicles; rV++) {
                    ...
                    if (routeV->nbCustomers == 0 || routeU->cour >= routeV->cour) { continue; }

                    if (loopID > 0 &&
                        std::max(routeU->whenLastModified, routeV->whenLastModified) <= lastTestLargeNbRouteU) {
                        continue;
                    }
...
bool HGSLocalSearch::MoveSingleClient() {
    ...
    double costSuppU = params->timeCost.get(nodeUPrevIndex, nodeXIndex) -
                       params->timeCost.get(nodeUPrevIndex, nodeUIndex) - params->timeCost.get(nodeUIndex, nodeXIndex);
    double costSuppV = params->timeCost.get(nodeVIndex, nodeUIndex) + params->timeCost.get(nodeUIndex, nodeYIndex) -
    ...
    if (routeU != routeV) {
        ...
        costSuppU += penaltyExcessLoad(routeU->load - loadU) + penaltyTimeWindows(routeUTwData) - routeU->penalty;

        costSuppV += penaltyExcessLoad(routeV->load + loadU) + penaltyTimeWindows(routeVTwData) - routeV->penalty;
    } else {
        ...
        if (nodeU->position < nodeV->position) {
            ...
            routeUTwData = MergeTWDataRecursive(nodeU->prev->prefixTwData, getRouteSegmentTwData(nodeX, nodeV),
            ...
            routeUTwData = MergeTWDataRecursive(nodeV->prefixTwData, nodeU->twData,
                                                getRouteSegmentTwData(nodeY, nodeU->prev), nodeX->postfixTwData);
        }
...
bool HGSLocalSearch::MoveTwoClientsReversed() {
    ...
    double costSuppU = params->timeCost.get(nodeUPrevIndex, nodeXNextIndex) -
                       params->timeCost.get(nodeUPrevIndex, nodeUIndex) - params->timeCost.get(nodeUIndex, nodeXIndex) -
                       params->timeCost.get(nodeXIndex, nodeXNextIndex);
    double costSuppV = params->timeCost.get(nodeVIndex, nodeXIndex) + params->timeCost.get(nodeXIndex, nodeUIndex) +
                       params->timeCost.get(nodeUIndex, nodeYIndex) - params->timeCost.get(nodeVIndex, nodeYIndex);
    TimeWindowData routeUTwData, routeVTwData;
...
bool HGSLocalSearch::SwapTwoSingleClients() {
    ...
    double costSuppU = params->timeCost.get(nodeUPrevIndex, nodeVIndex) + params->timeCost.get(nodeVIndex, nodeXIndex) -
                       params->timeCost.get(nodeUPrevIndex, nodeUIndex) - params->timeCost.get(nodeUIndex, nodeXIndex);
    double costSuppV = params->timeCost.get(nodeVPrevIndex, nodeUIndex) + params->timeCost.get(nodeUIndex, nodeYIndex) -
                       params->timeCost.get(nodeVPrevIndex, nodeVIndex) - params->timeCost.get(nodeVIndex, nodeYIndex);
    TimeWindowData routeUTwData, routeVTwData;
    ...
    if (routeU != routeV) {
        ...
        costSuppU +=
            penaltyExcessLoad(routeU->load + loadV - loadU) + penaltyTimeWindows(routeUTwData) - routeU->penalty;

        costSuppV +=
            penaltyExcessLoad(routeV->load + loadU - loadV) + penaltyTimeWindows(routeVTwData) - routeV->penalty;
    } else {
        if (!routeUTimeWarp && costSuppU + costSuppV > -MY_EPSILON) { return false; }
        ...
        if (nodeU->position < nodeV->position) {
            ...
            routeUTwData =
                MergeTWDataRecursive(nodeU->prev->prefixTwData, nodeV->twData,
                                     getRouteSegmentTwData(nodeX, nodeV->prev), nodeU->twData, nodeY->postfixTwData);
        } else {
            // Edge case U directly after V is excluded from beginning of function
            // start - ... - VPrev - U - Y - ... - UPrev - V - X - ... - end
            routeUTwData =
                MergeTWDataRecursive(nodeV->prev->prefixTwData, nodeU->twData,
                                     getRouteSegmentTwData(nodeY, nodeU->prev), nodeV->twData, nodeX->postfixTwData);
        }
...
bool HGSLocalSearch::SwapTwoClientsForOne() {
    ...
    double costSuppU =
        params->timeCost.get(nodeUPrevIndex, nodeVIndex) + params->timeCost.get(nodeVIndex, nodeXNextIndex) -
        params->timeCost.get(nodeUPrevIndex, nodeUIndex) - params->timeCost.get(nodeXIndex, nodeXNextIndex);
    double costSuppV = params->timeCost.get(nodeVPrevIndex, nodeUIndex) + params->timeCost.get(nodeXIndex, nodeYIndex) -
                       params->timeCost.get(nodeVPrevIndex, nodeVIndex) - params->timeCost.get(nodeVIndex, nodeYIndex);
    TimeWindowData routeUTwData, routeVTwData;
    ...
    if (routeU != routeV) {
        ...
        routeVTwData =
            MergeTWDataRecursive(nodeV->prev->prefixTwData, getEdgeTwData(nodeU, nodeX), nodeY->postfixTwData);

        costSuppU += penaltyExcessLoad(routeU->load + loadV - loadU - loadX) + penaltyTimeWindows(routeUTwData) -
                     routeU->penalty;

        costSuppV += penaltyExcessLoad(routeV->load + loadU + loadX - loadV) + penaltyTimeWindows(routeVTwData) -
                     routeV->penalty;
    } else {
        if (!routeUTimeWarp && costSuppU + costSuppV > -MY_EPSILON) { return false; }
        ...
        if (nodeU->position < nodeV->position) {
            ...
            routeUTwData = MergeTWDataRecursive(nodeV->prev->prefixTwData, getEdgeTwData(nodeU, nodeX),
    ...
    // Note: next two lines are a bit inefficient but we only update occasionally and updateRouteData is much more
    // costly anyway, efficient checks are more important
    swapNode(nodeU, nodeV);
    insertNode(nodeX, nodeU);
    nbMoves++; // Increment move counter before updating route data
    searchCompleted = false;
...
bool HGSLocalSearch::SwapTwoClientPairs() {
    ...
    double costSuppU =
        params->timeCost.get(nodeUPrevIndex, nodeVIndex) + params->timeCost.get(nodeYIndex, nodeXNextIndex) -
        params->timeCost.get(nodeUPrevIndex, nodeUIndex) - params->timeCost.get(nodeXIndex, nodeXNextIndex);
    double costSuppV =
        params->timeCost.get(nodeVPrevIndex, nodeUIndex) + params->timeCost.get(nodeXIndex, nodeYNextIndex) -
        params->timeCost.get(nodeVPrevIndex, nodeVIndex) - params->timeCost.get(nodeYIndex, nodeYNextIndex);
    TimeWindowData routeUTwData, routeVTwData;
    ...
    if (routeU != routeV) {
        ...
        routeUTwData =
            MergeTWDataRecursive(nodeU->prev->prefixTwData, getEdgeTwData(nodeV, nodeY), nodeX->next->postfixTwData);
        routeVTwData =
            MergeTWDataRecursive(nodeV->prev->prefixTwData, getEdgeTwData(nodeU, nodeX), nodeY->next->postfixTwData);
...
bool HGSLocalSearch::TwoOptWithinTrip() {
    ...
    double cost = params->timeCost.get(nodeUIndex, nodeVIndex) + params->timeCost.get(nodeXIndex, nodeYIndex) -
                  params->timeCost.get(nodeUIndex, nodeXIndex) - params->timeCost.get(nodeVIndex, nodeYIndex) +
                  nodeV->cumulatedReversalDistance - nodeX->cumulatedReversalDistance;

    if (!routeUTimeWarp && cost > -MY_EPSILON) { return false; }
...
bool HGSLocalSearch::swapStar(const bool withTW) {
    ...
    for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next) {
        for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next) {
            ...
            const double loadPenU =
                penaltyExcessLoad(routeU->load + params->cli[nodeV->cour].demand - params->cli[nodeU->cour].demand);
            const double loadPenV =
                penaltyExcessLoad(routeV->load + params->cli[nodeU->cour].demand - params->cli[nodeV->cour].demand);
            const double deltaLoadPen =
                loadPenU + loadPenV - penaltyExcessLoad(routeU->load) - penaltyExcessLoad(routeV->load);
            const int deltaRemoval =
                withTW ? nodeU->deltaRemovalTW + nodeV->deltaRemovalTW : nodeU->deltaRemoval + nodeV->deltaRemoval;

            // Quick filter: possibly early elimination of many SWAP* due to the capacity constraints/penalties and
            // bounds on insertion costs
    ...
    if (!myBestSwapStar.bestPositionU || !myBestSwapStar.bestPositionV) { return false; }
    ...
    TimeWindowData routeUTwData, routeVTwData;
    // It is not possible to have bestPositionU == V or bestPositionV == U, so the positions are always strictly
    // different
    if (myBestSwapStar.bestPositionV->position == myBestSwapStar.U->position - 1) {
        // Special case
        routeUTwData = MergeTWDataRecursive(myBestSwapStar.bestPositionV->prefixTwData, myBestSwapStar.V->twData,
                                            myBestSwapStar.U->next->postfixTwData);
    } else if (myBestSwapStar.bestPositionV->position < myBestSwapStar.U->position) {
        routeUTwData =
            MergeTWDataRecursive(myBestSwapStar.bestPositionV->prefixTwData, myBestSwapStar.V->twData,
                                 getRouteSegmentTwData(myBestSwapStar.bestPositionV->next, myBestSwapStar.U->prev),
                                 myBestSwapStar.U->next->postfixTwData);
    } else {
        routeUTwData =
            MergeTWDataRecursive(myBestSwapStar.U->prev->prefixTwData,
    ...
    if (myBestSwapStar.bestPositionU->position == myBestSwapStar.V->position - 1) {
        // Special case
        routeVTwData = MergeTWDataRecursive(myBestSwapStar.bestPositionU->prefixTwData, myBestSwapStar.U->twData,
                                            myBestSwapStar.V->next->postfixTwData);
    } else if (myBestSwapStar.bestPositionU->position < myBestSwapStar.V->position) {
        routeVTwData =
            MergeTWDataRecursive(myBestSwapStar.bestPositionU->prefixTwData, myBestSwapStar.U->twData,
                                 getRouteSegmentTwData(myBestSwapStar.bestPositionU->next, myBestSwapStar.V->prev),
                                 myBestSwapStar.V->next->postfixTwData);
    } else {
        routeVTwData =
            MergeTWDataRecursive(myBestSwapStar.V->prev->prefixTwData,
...
int HGSLocalSearch::getCheapestInsertSimultRemoval(Node *U, Node *V, Node *&bestPosition) {
    ...
    int deltaCost = params->timeCost.get(V->prev->cour, U->cour) + params->timeCost.get(U->cour, V->next->cour) -
...
int HGSLocalSearch::getCheapestInsertSimultRemovalWithTW(Node *U, Node *V, Node *&bestPosition) {
...
void HGSLocalSearch::preprocessInsertions(HGSRoute *R1, HGSRoute *R2) {
    for (Node *U = R1->depot->next; !U->isDepot; U = U->next) {
        ...
        U->deltaRemoval = params->timeCost.get(U->prev->cour, U->next->cour) -
                          params->timeCost.get(U->prev->cour, U->cour) - params->timeCost.get(U->cour, U->next->cour);
        ...
        if (R2->whenLastModified > currentOption.whenLastCalculated) {
            ...
            currentOption.bestCost[0]        = params->timeCost.get(0, U->cour) +
            ...
            for (Node *V = R2->depot->next; !V->isDepot; V = V->next) {
                int deltaCost = params->timeCost.get(V->cour, U->cour) + params->timeCost.get(U->cour, V->next->cour) -
...
void HGSLocalSearch::preprocessInsertionsWithTW(HGSRoute *R1, HGSRoute *R2) {
    ...
    for (Node *U = R1->depot->next; !U->isDepot; U = U->next) {
        ...
        if (R1->isDeltaRemovalTWOutdated) {
            ...
            U->deltaRemovalTW =
                params->timeCost.get(U->prev->cour, U->next->cour) - params->timeCost.get(U->prev->cour, U->cour) -
                params->timeCost.get(U->cour, U->next->cour) + deltaPenaltyTimeWindows(twData, R1->twData);
        }
        ...
        if (R2->whenLastModified > currentOption.whenLastCalculated) {
            ...
            currentOption.bestCost[0] =
                params->timeCost.get(0, U->cour) + params->timeCost.get(U->cour, R2->depot->next->cour) -
                params->timeCost.get(0, R2->depot->next->cour) + deltaPenaltyTimeWindows(twData, R2->twData);
...
void HGSLocalSearch::updateRouteData(HGSRoute *myRoute) {
    ...
    while (!mynode->isDepot || firstIt) {
        ...
        myReversalDistance += params->timeCost.get(mynode->cour, mynode->prev->cour) -
...
HGSLocalSearch::HGSLocalSearch(Params *params) : params(params) {
    ...
    bestInsertClient              = std::vector<std::vector<ThreeBestInsert>>(params->nbVehicles,
                                                                              std::vector<ThreeBestInsert>(params->nbClients + 1));
    bestInsertClientTW            = std::vector<std::vector<ThreeBestInsert>>(params->nbVehicles,
                                                                              std::vector<ThreeBestInsert>(params->nbClients + 1));
