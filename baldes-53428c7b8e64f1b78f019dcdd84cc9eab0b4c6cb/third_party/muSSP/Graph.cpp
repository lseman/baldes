...
Graph::Graph(int num_nodes, int num_edges, int src_id, int sink_id, double en_weight, double ex_weight) {
    ...
    V_ = std::vector<SSPNode>(num_nodes);
...
SSPNode &Graph::get_node(int node_id) { return V_[node_id]; }
...
void Graph::add_edge(int tail_id, int head_id, int edge_id, double weight) {
    ...
    if (false) {
        ...
        if (static_cast<int>(edge_weights.size()) - 1 != edge_id) std::cout << "we got wrong edge number" << std::endl;
    }
}
...
void Graph::topologicalSortUtil(int v, std::vector<bool> &visited, std::stack<int> &Stack) {
    if (v < 0 || v >= V_.size()) {
        std::cerr << "Invalid node index: " << v << std::endl;
        return;
    }
    ...
    for (auto node_id : V_[v].successor_idx) {
        if (node_id < 0 || node_id >= V_.size()) {
            std::cerr << "Invalid successor index: " << node_id << " for node " << v << std::endl;
            continue;
        }

        if (!visited[node_id]) { topologicalSortUtil(node_id, visited, Stack); }
    }
...
void Graph::shortest_path_dag() {
    ...
    for (int i = 1; i < num_nodes_; i += 2) distance2src[i] = V_[i].precursor_edges_weights[0];
    for (int i = 2; i < num_nodes_; i += 2) distance2src[i] = FINF;
    ...
    while (!Stack.empty()) {
        ...
        ancestor_node_id[cur_node_id] =
            src_id_ == parent_node_id[cur_node_id] ? cur_node_id : ancestor_node_id[parent_node_id[cur_node_id]];

        if (cur_node_id != sink_id_) ancestors_descendants[ancestor_node_id[cur_node_id]].push_back(cur_node_id);
    ...
    //// update price
    for (int i = 0; i < this->num_nodes_; i++) { V_[i].price -= distance2src[i]; }
}
...
void Graph::update_allgraph_weights() {
    ...
    for (int i = 2; i < num_nodes_; i += 2) {
        ancestor_ssd[ancestor_node_id[i]] =
            MIN(ancestor_ssd[ancestor_node_id[i]], sink_info->sink_precursor_weights[i]);
    }
...
void Graph::extract_shortest_path() {
    ...
    while (tmp_node_id != src_id_) {
        if (tmp_node_id < 0 || tmp_node_id >= parent_node_id.size()) {
            std::cerr << "Invalid node ID in path extraction: " << tmp_node_id << " (size: " << parent_node_id.size()
                      << ")" << std::endl;
            return; // Abort path extraction
        }
...
void Graph::topologicalSort_counter_order(int v) {
    // Check for invalid node index
    if (v < 0 || v >= node_in_visited.size()) {
        std::cerr << "Invalid node index: " << v << std::endl;
        return;
    }
    ...
    // Recur for the parent node if it exists
    if (parent_node_id[v] >= 0 && parent_node_id[v] < node_in_visited.size() && !node_in_visited[parent_node_id[v]]) {
        topologicalSort_counter_order(parent_node_id[v]);
    }

    // Push current vertex to the topological order vector
    ...
    for (size_t j = 0; j < V_[curr_node_id].successor_idx.size(); j++) {
        ...
        if (node_in_visited[it] > 0) {
            double cur_edge_weight =
                V_[curr_node_id].successor_edges_weights[j] - V_[curr_node_id].price + V_[it].price;
...
void Graph::update_shortest_path_tree_recursive(std::vector<int> &update_node_id) {
    ...
    if (precursor_queue_top_val < curr_best_choice->first) {
        ...
        while (!node_upt_waitinglist.empty()) {
            multi_map_it =
                node_upt_waitinglist.begin(); ////Current vertex. The shortest distance for this has been found
            ...
            if (!node_in_visited[curr_node_id]) ////If the vertex is already visited, no point in exploring adjacent
                                                ///vertices
                continue;
            //// update ancestor
...
void Graph::update_subgraph_weights(std::vector<int> &update_node_id) {
    ...
    for (auto &&i : update_node_id) { distance2src[i] = 0; }
}
