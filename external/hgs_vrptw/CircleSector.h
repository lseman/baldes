...
// Angles are measured in [0,65535] instead of [0,359], in such a way that modulo operations are much faster (since 2^16
// = 65536) Credit to Fabian Giesen at
...
struct CircleSector {
    ...
    static int positive_mod(int i) {
        ...
        // Remark that "i % 65536" should be automatically compiled in an optimized form as "i & 0xffff" for faster
        // calculations
    ...
    bool isEnclosed(int point) { return (positive_mod(point - start) <= positive_mod(end - start)); }

    // Tests overlap of two circle sectors with tolerance
    // Note, this effectively is sector1.isEnclosed(sector2.start) || sector2.isEnclosed(sector1.start), while also
    ...
    static bool overlap(const CircleSector &sector1, const CircleSector &sector2, const int tolerance) {
        // the RHS is the size of the sector, by adding the tolerance outside of the positive_mod, we avoid overflow
