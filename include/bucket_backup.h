...
struct alignas(64) Bucket {
    ...
    void add_label(Label* label) noexcept {
        if (!label) return;
        is_split ? add_label<BucketState::Split>(label)
                 : add_label<BucketState::Unsplit>(label);
    }

    void add_sorted_label(Label* label) noexcept {
        if (!label) return;
        is_split ? add_sorted_label<BucketState::Split>(label)
    ...
    Label* get_best_label() noexcept {
        return is_split ? get_best_label<BucketState::Split>()
                        : get_best_label<BucketState::Unsplit>();
    }

    double get_cb() const noexcept {
        if (labels_vec.empty() && !is_split)
            return std::numeric_limits<double>::max();
        return is_split ? get_cb<BucketState::Split>()
                        : get_cb<BucketState::Unsplit>();
    }

    bool check_dominance(const Label* new_label, const auto& dominance_func,
                         int& stat_n_dom) const noexcept {
        if (!new_label || get_cb() > new_label->cost) return false;

        if (is_split) {
            if (labels_vec.empty()) return false;

            const double resource_val = new_label->resources[RESOURCE_INDEX];
            if (resource_val < lb[RESOURCE_INDEX] ||
                resource_val > ub[RESOURCE_INDEX]) {
                return false;
            }

            const bool check_first = resource_val <= split_midpoint;
            const bool check_second = resource_val >= split_midpoint;

            if (check_first && split_point > 0) {
                std::vector<Label*> first_bucket(
                    labels_vec.begin(), labels_vec.begin() + split_point);
                if (dominance_func(first_bucket)) {
                    ++stat_n_dom;
                    return true;
                }
            }

            if (check_second && split_point < labels_vec.size()) {
                std::vector<Label*> second_bucket(
                    labels_vec.begin() + split_point, labels_vec.end());
                if (dominance_func(second_bucket)) {
                    ++stat_n_dom;
                    return true;
                }
            }
    ...
    void lazy_flush(Label* new_label) noexcept {
        if (new_label) labels_flush.push_back(new_label);
    }

   private:
    auto find_insert_position(Label* label, size_t start, size_t end) const {
        if (!label || start >= labels_vec.size() || end > labels_vec.size() ||
            start >= end) {
            return labels_vec.end();
        }

        return std::ranges::lower_bound(labels_vec.begin() + start,
                                        labels_vec.begin() + end, label,
    ...
    void ensure_capacity() noexcept {
        if (labels_vec.size() == labels_vec.capacity()) {
            labels_vec.reserve(
                std::max<size_t>(MIN_RESERVE_SIZE, labels_vec.capacity() * 2));
        }
    }

    bool should_split() const noexcept {
        return depth < MAX_BUCKET_DEPTH &&
               labels_vec.size() >= BUCKET_CAPACITY && !is_split;
    }
    ...
    void add_label(Label* label) noexcept {
        ...
        if constexpr (State == BucketState::Split) {
            ...
            const bool goes_to_first =
                label->resources[RESOURCE_INDEX] <= split_midpoint;
            ...
            const size_t old_size = labels_vec.size();
            labels_vec.insert(insert_pos, label);
            if (goes_to_first && labels_vec.size() > old_size) {
                split_point++;
            }
        } else {
            ensure_capacity();
    ...
    Label* get_best_label() noexcept {
        if constexpr (State == BucketState::Split) {
            Label* first_best =
                split_point > 0 ? get_min_label(0, split_point) : nullptr;
            Label* second_best =
                split_point < labels_vec.size()
                    ? get_min_label(split_point, labels_vec.size())
                    : nullptr;
    ...
    double get_cb() const noexcept {
        if constexpr (State == BucketState::Split) {
            ...
            if (split_point > 0) {
                min_cb = std::min(min_cb, get_min_cost(0, split_point));
            }
            if (split_point < labels_vec.size()) {
                min_cb = std::min(min_cb,
                                  get_min_cost(split_point, labels_vec.size()));
            }
            return min_cb;
        } else {
#ifdef SORTED_LABELS
    ...
    void split_into_virtual_sub_buckets() noexcept {
        ...
        auto [min_it, max_it] =
            std::minmax_element(labels_vec.begin(), labels_vec.end(),
                                [](const Label* a, const Label* b) {
                                    return a->resources[RESOURCE_INDEX] <
                                           b->resources[RESOURCE_INDEX];
                                });

        split_midpoint = ((*min_it)->resources[RESOURCE_INDEX] +
                          (*max_it)->resources[RESOURCE_INDEX]) /
                         2;

        auto partition_point = std::partition(
            labels_vec.begin(), labels_vec.end(),
            [this](const Label* label) noexcept {
    ...
    std::pair<std::vector<Label*>, std::vector<Label*>> partition_removals(
        const std::vector<Label*>& to_remove) const noexcept {
    ...
    void remove_labels_batch(const std::vector<Label*>& to_remove) noexcept {
        ...
        if constexpr (State == BucketState::Split) {
            auto [bucket0_removes, bucket1_removes] =
                partition_removals(to_remove);
            if (!bucket0_removes.empty()) {
                remove_from_virtual_bucket(bucket0_removes, 0, split_point);
            }
            if (!bucket1_removes.empty()) {
                remove_from_virtual_bucket(bucket1_removes, split_point,
                                           labels_vec.size());
            }
        } else {
#ifdef SORTED_LABELS
...
#else
            to_remove.size() > SMALL_BATCH_THRESHOLD
                ? remove_batch_hash_range(to_remove, 0, labels_vec.size())
                : remove_batch_linear_range(to_remove, 0, labels_vec.size());
#endif
        }
    }

    void remove_from_virtual_bucket(const std::vector<Label*>& to_remove,
...
#else
        to_remove.size() > SMALL_BATCH_THRESHOLD
            ? remove_batch_hash_range(to_remove, start, end)
            : remove_batch_linear_range(to_remove, start, end);
#endif
    }

    void remove_batch_sorted_range(const std::vector<Label*>& to_remove,
                                   size_t start, size_t end) noexcept {
        if (start >= labels_vec.size() || end > labels_vec.size() ||
            start >= end) {
            return;
        }
        ...
        for (Label* label : to_remove) {
            ...
            auto it = std::ranges::lower_bound(
                labels_vec.begin() + start, labels_vec.begin() + end, label,
                [](const Label* a, const Label* b) {
                    return a->cost < b->cost;
                });
            if (it != labels_vec.begin() + end && *it == label) {
                size_t idx = std::distance(labels_vec.begin() + start, it);
                should_remove[idx] = true;
            }
        }

        size_t write = start;
        for (size_t read = start; read < end && read < labels_vec.size();
             read++) {
            if (!should_remove[read - start]) {
                if (write != read) {
                    labels_vec[write] = labels_vec[read];
                }
                write++;
            }
        }
        ...
        if (write < end) {
            std::move(labels_vec.begin() + end, labels_vec.end(),
                      labels_vec.begin() + write);
            labels_vec.resize(labels_vec.size() - (end - write));
        }
    }

    void remove_batch_hash_range(const std::vector<Label*>& to_remove,
                                 size_t start, size_t end) noexcept {
        ankerl::unordered_dense::set<Label*> remove_set(to_remove.begin(),
                                                        to_remove.end());
        size_t write = start;
    ...
    void remove_batch_linear_range(const std::vector<Label*>& to_remove,
                                   size_t start, size_t end) noexcept {
        for (Label* label : to_remove) {
            if (!label) continue;
            auto it = std::find(labels_vec.begin() + start,
                                labels_vec.begin() + end, label);
    ...
    double get_min_cost(size_t start, size_t end) const noexcept {
        ...
        auto min_it = std::ranges::min_element(
            labels_vec.begin() + start, labels_vec.begin() + end,
            [](const Label* a, const Label* b) {
                if (!a || !b) return false;
                return a->cost < b->cost;
            });

        return (min_it != labels_vec.begin() + end && *min_it)
                   ? (*min_it)->cost
    ...
    Label* get_min_label(size_t start, size_t end) noexcept {
        ...
        return (min_it != labels_vec.begin() + end && *min_it) ? *min_it
                                                               : nullptr;
    }
    ...
    void remove_label_impl(Label* label, int recursion_depth) noexcept {
        ...
        if (is_split) {
            ...
            if (label->resources[RESOURCE_INDEX] <= split_midpoint) {
                auto it = std::find(labels_vec.begin(),
                                    labels_vec.begin() + split_point, label);
                if (it != labels_vec.begin() + split_point) {
                    labels_vec.erase(it);
                    split_point--;
                }
            } else {
                auto it = std::find(labels_vec.begin() + split_point,
                                    labels_vec.end(), label);
    ...
    void add_sorted_label(Label* label) noexcept {
        ...
        if constexpr (State == BucketState::Split) {
            ...
            if (label->cost >= labels_vec.back()->cost) {
                ...
                auto insert_pos =
                    find_insert_position(label, 0, labels_vec.size());
                labels_vec.insert(insert_pos, label);
            }
