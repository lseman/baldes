...
public:
    ...
    srex_crossover(const std::vector<uint16_t> &parent1, const std::vector<uint16_t> &parent2, int i, int j) {
        int n = parent1.size();
     ...
     */
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>>
    relocate_star(const std::vector<uint16_t> &route1, const std::vector<uint16_t> &route2, int i, int j) {
     ...
     */
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>>
    enhanced_swap(const std::vector<uint16_t> &route1, const std::vector<uint16_t> &route2, int i, int j) {
        // Check if indices are valid and do not involve depots
     ...
     *
     */
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>> cross(const std::vector<uint16_t> &route1,
     ...
     */
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>>
    insertion(const std::vector<uint16_t> &route1, const std::vector<uint16_t> &route2, int k, int l) {
     ...
     *
     */
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>> swap(const std::vector<uint16_t> &route1,
                                                                 const std::vector<uint16_t> &route2, int k, int l) {
    ...
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>>
    nm_exchange(const std::vector<uint16_t> &route1, const std::vector<uint16_t> &route2, int i, int j) {
    ...
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>>
    extended_swap_star(const std::vector<uint16_t> &route1, const std::vector<uint16_t> &route2, int i, int j) {
    ...
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>> extended_swap_star_fun(const std::vector<uint16_t> &route1,
                                                                                   const std::vector<uint16_t> &route2,
                                                                                   int i, int j, int chain_length = 2) {
    ...
    std::pair<std::vector<uint16_t>, std::vector<uint16_t>> apply_operator(int                          op_index,
                                                                           const std::vector<uint16_t> &route1,
                                                                           const std::vector<uint16_t> &route2, int k,
                                                                           int l) {
    ...
    void update_operator_weights(double decay_factor = 0.9, double reward_factor = 0.1, double min_weight = 0.01) {
        ...
        if (total_improvement > 0 || total_successes > 0) {
            ...
            for (size_t i = 0; i < n; ++i) {
                double normalized_improvement = operator_improvements[i] * inv_total_improvement;
                double success_rate           = operator_success_count[i] * inv_total_successes;
                double new_weight =
                    decay_factor * operator_weights[i] + reward_factor * (normalized_improvement + success_rate);
                // Ensure a minimum weight.
                operator_weights[i] = (new_weight < min_weight) ? min_weight : new_weight;
                total_weight += operator_weights[i];
            }

            // Normalize the weights so that they sum to 1.
            if (total_weight > 0.0) {
                for (size_t i = 0; i < n; ++i) { operator_weights[i] /= total_weight; }
            }
        }
    ...
    exec::static_thread_pool            pool  = exec::static_thread_pool(std::thread::hardware_concurrency() / 4);
    ...
    std::vector<Label *> perturbation(const std::vector<Label *> &paths) {
        ...
        tasks.reserve(paths.size() * paths.size() * 4); // adjust estimate as needed
        ...
        auto bulk_sender = stdexec::bulk(
            stdexec::just(), num_chunks, [&tasks, chunk_size, &local_best_new, this](std::size_t chunk_idx) {
                ...
                for (size_t task_idx = start_idx; task_idx < end_idx; ++task_idx) {
                    ...
                    auto add_improved_label = [&](const Path &path, const std::pair<double, double> &cost,
        ...
        for (auto &vec : local_best_new) { best_new.insert(best_new.end(), vec.begin(), vec.end()); }
...
private:
    ...
    bool is_feasible(const Path &route) const {
        ...
        if (route_size < 2 || route.route.front() != 0 || route.route.back() != N_SIZE - 1) { return false; }
        ...
        for (size_t i = 0; i < route_size - 1; ++i) {
            ...
            if (visited[segment] & (1ULL << bit_position)) { return false; }
            ...
            const double travel_time  = source.duration + instance.getcij(source_id, target_id);
            const double arrival_time = std::max(static_cast<double>(target.lb[0]), current_time + travel_time);

            // Check the time window for the target node.
            if (arrival_time > target.ub[0]) { return false; }

            // Update current time and capacity.
            current_time = arrival_time;
            current_capacity += source.demand;
            if (current_capacity > total_capacity) { return false; }
        }
        return true;
    }
