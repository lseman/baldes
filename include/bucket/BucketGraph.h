...
class BucketGraph {
   ...
   public:
    ...
    inline bool is_within_bounds(const BucketRange<D> &new_range,
                                 const BucketRange<D> &fixed_range) {
        return (new_range.lower_bound >= fixed_range.lower_bound &&
                new_range.upper_bound <= fixed_range.upper_bound);
    }
    ...
    ankerl::unordered_dense::map<int, BucketIntervalTree<D>> rebuild_buckets() {
        ...
        // auto &fixed_buckets = assign_buckets<D>(fw_fixed_buckets,
        // bw_fixed_buckets);
        ...
        for (int from_bucket = 0; from_bucket < fw_buckets_size;
            ...
            for (int to_bucket = 0; to_bucket < fw_buckets_size; ++to_bucket) {
                ...
                // Check if there is a fixed arc between from_bucket and
                // to_bucket
                ...
                const uint64_t bit =
                    1ULL << (pos & 63);  // Bit position within the uint64_t

                // Check if the bit is set (i.e., the arc is fixed)
                if ((fixed_buckets_bitmap[idx] & bit) != 0)
                    continue;  // Skip fixed arcs

                // Get the range for from_bucket
                BucketRange<D> from_range = {buckets[from_bucket].lb,
                                             buckets[from_bucket].ub};

                // Get the range for to_bucket
                BucketRange<D> to_range = {buckets[to_bucket].lb,
                                           buckets[to_bucket].ub};
     ...
     *
     * This function processes the buckets in the specified direction (Forward
     * or Backward) to identify fixed arcs based on heritages. It iterates
     * through each bucket and checks if the node exists in the save_rebuild
     * map. If the node is found, it searches for arcs and updates the fixed
     * buckets accordingly.
     *
     */
    template <Direction D>
    void process_buckets(auto buckets_size, auto &buckets, auto &save_rebuild,
                         auto &fixed_buckets) {
        std::vector<int> tasks;
        std::atomic<int> n_fixed =
            0;  // Declare n_fixed as atomic for thread safety
        ...
        for (int bucket = 0; bucket < buckets_size; ++bucket) {
            tasks.push_back(bucket);
        }

        // Define chunk size to reduce parallelization overhead
        const int chunk_size =
            50;  // You can adjust this based on performance experiments
        ...
        auto bulk_sender = stdexec::bulk(
            ...
            [&, chunk_size](std::size_t chunk_idx) {
                ...
                for (size_t task_idx = start_idx; task_idx < end_idx;
                    ...
                    BucketRange<D> from_range = {from_bucket.lb,
                                                 from_bucket.ub};
                    auto from_node = from_bucket.node_id;

                    // Check if the node exists in the save_rebuild map (only
                    // once per bucket)
                    ...
                    if (it != save_rebuild.end()) {
                        ...
                        for (auto &arc : bucket_arcs) {
                            ...
                            auto is_contained =
                                save_tree.search(from_range, to_range, to_node);
                            if (is_contained) {
    ...
    void redefine(int bucketInterval) {
        ...
        PARALLEL_SECTIONS(
            ...
            SECTION {
                ...
                fw_fixed_buckets_bitmap.resize(
                    (n * n + 63) >> 6,
                    0);  // Ensure fixed_buckets_bitmap is the correct size
            },
            SECTION {
                ...
                bw_fixed_buckets_bitmap.resize(
                    (n * n + 63) >> 6,
                    0);  // Ensure fixed_buckets_bitmap is the correct size
            });
    ...
    void reset_fixed_buckets() {
        std::fill(fw_fixed_buckets_bitmap.begin(),
                  fw_fixed_buckets_bitmap.end(), 0);
        std::fill(bw_fixed_buckets_bitmap.begin(),
                  bw_fixed_buckets_bitmap.end(), 0);
    }
    ...
    void updateSplit() {
        ...
        //////////////////////////////////////////////////////////////////////
        // Adjust the terminal time dynamically based on the difference between
        // the number of forward and backward labels
        for (auto &split : q_star) {
            // print n_fw_labels and n_bw_labels
            // If there are more backward labels than forward labels, increase
            // the terminal time slightly
            if (((static_cast<double>(n_bw_labels) -
                  static_cast<double>(n_fw_labels)) /
                 static_cast<double>(n_fw_labels)) > 0.2) {
                split += 0.05 * R_max[options.main_resources[0]];
            }
            // If there are more forward labels than backward labels, decrease
            // the terminal time slightly
            else if (((static_cast<double>(n_fw_labels) -
                       static_cast<double>(n_bw_labels)) /
    ...
    bool DominatedInCompWiseSmallerBuckets(
        const Label *L, int bucket, const std::vector<double> &c_bar,
