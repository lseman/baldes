...
class BucketGraph {
    ...
    static constexpr uint64_t bit_mask_lookup[64] = {
        UINT64_C(1) << 0,  UINT64_C(1) << 1,  UINT64_C(1) << 2,  UINT64_C(1) << 3,  UINT64_C(1) << 4,
        UINT64_C(1) << 5,  UINT64_C(1) << 6,  UINT64_C(1) << 7,  UINT64_C(1) << 8,  UINT64_C(1) << 9,
        UINT64_C(1) << 10, UINT64_C(1) << 11, UINT64_C(1) << 12, UINT64_C(1) << 13, UINT64_C(1) << 14,
        UINT64_C(1) << 15, UINT64_C(1) << 16, UINT64_C(1) << 17, UINT64_C(1) << 18, UINT64_C(1) << 19,
        UINT64_C(1) << 20, UINT64_C(1) << 21, UINT64_C(1) << 22, UINT64_C(1) << 23, UINT64_C(1) << 24,
        UINT64_C(1) << 25, UINT64_C(1) << 26, UINT64_C(1) << 27, UINT64_C(1) << 28, UINT64_C(1) << 29,
        UINT64_C(1) << 30, UINT64_C(1) << 31, UINT64_C(1) << 32, UINT64_C(1) << 33, UINT64_C(1) << 34,
        UINT64_C(1) << 35, UINT64_C(1) << 36, UINT64_C(1) << 37, UINT64_C(1) << 38, UINT64_C(1) << 39,
        UINT64_C(1) << 40, UINT64_C(1) << 41, UINT64_C(1) << 42, UINT64_C(1) << 43, UINT64_C(1) << 44,
        UINT64_C(1) << 45, UINT64_C(1) << 46, UINT64_C(1) << 47, UINT64_C(1) << 48, UINT64_C(1) << 49,
        UINT64_C(1) << 50, UINT64_C(1) << 51, UINT64_C(1) << 52, UINT64_C(1) << 53, UINT64_C(1) << 54,
        UINT64_C(1) << 55, UINT64_C(1) << 56, UINT64_C(1) << 57, UINT64_C(1) << 58, UINT64_C(1) << 59,
        UINT64_C(1) << 60, UINT64_C(1) << 61, UINT64_C(1) << 62, UINT64_C(1) << 63};
...
public:
     ...
     */
    void reset_fixed() {
        size_t bitmap_size = (fw_buckets.size() * fw_buckets.size() + 63) / 64; // Round up division by 64
    ...
    void updateSplit() {
        ...
        if (imbalance_ratio > IMBALANCE_THRESHOLD) {
            double adjustment = ADJUSTMENT_FACTOR * R_max[options.main_resources[0]];

            // Adjust split time based on which direction has more labels
            for (auto &split : q_star) { split += (bw_labels > fw_labels) ? adjustment : -adjustment; }
        }
    }
    ...
    bool DominatedInCompWiseSmallerBuckets(const Label *L, int bucket, const std::vector<double> &c_bar,
                                           ...
                                           const std::vector<std::vector<int>> &bucket_order, int &stat_n_dom) noexcept;
    ...
    void set_deleted_arcs(const std::vector<std::pair<int, int>> &arcs) {
        ...
        for (const auto &[from, to] : arcs) {
            if (from < num_nodes && to < num_nodes) { set_arc_fixed(from, to); }
        }
    }
    ...
    Label *compute_red_cost(Label *L, bool fw) {
        ...
        for (auto node_id : L->nodes_covered) {
            ...
            for (size_t i = 0; i < limit; ++i) {
                uint64_t current_visited = new_label->visited_bitmap[i]; // Get visited nodes in the
                                                                         // current segment

                if (!current_visited) continue; // Skip if no nodes were visited in this segment

                uint64_t neighborhood_mask = neighborhoods_bitmap[node_id][i]; // Get neighborhood mask
    ...
    bool shallUpdateStep() {
        ...
        for (size_t b = 0; b < fw_buckets.size(); b++) {
            if (fw_buckets[b].shall_split) {
                ...
                if (inner_id != options.depot && inner_id != options.end_depot) {
                    if (updated_buckets.contains(inner_id)) { continue; }
                    auto new_val = fw_bucket_splits[inner_id] * 2;
                    if (new_val > 200) { continue; }
        ...
        for (size_t b = 0; b < bw_buckets.size(); b++) {
            if (bw_buckets[b].shall_split) {
                ...
                if (inner_id != options.depot && inner_id != options.end_depot) {
                    if (updated_buckets.contains(inner_id)) { continue; }
                    auto new_val = bw_bucket_splits[inner_id] * 2;
                    if (new_val > 200) { continue; }
                    bw_bucket_splits[inner_id] = new_val;
                    fw_bucket_splits[inner_id] = new_val;
                    updated_buckets.insert(inner_id);
