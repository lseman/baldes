...
[[nodiscard]] constexpr bool process_disposable_resource(
    double& new_resource, const double initial_resource, const double increment,
    const auto& theNode, const size_t I) {
...
template <Direction D>
[[nodiscard]] constexpr bool process_binary_resource(double& new_resource,
...
template <Direction D>
[[nodiscard]] bool process_mtw_resource(double& new_resource,
    ...
    if constexpr (D == Direction::Forward) {
        return std::any_of(theNode.mtw_lb.begin(), theNode.mtw_lb.end(),
                           [&, I](const auto& lb) {  // Capture I explicitly
                               new_resource =
                                   std::max(initial_resource + increment, lb);
                               return new_resource <= theNode.ub[I];
                           });
    } else {
        return std::any_of(theNode.mtw_ub.begin(), theNode.mtw_ub.end(),
                           [&, I](const auto& ub) {  // Capture I explicitly
                               new_resource =
                                   std::min(initial_resource - increment, ub);
                               return new_resource >= theNode.lb[I];
                           });
    }
}
...
[[nodiscard]] constexpr bool process_battery_resource(
    double& new_resource, const double initial_resource, const auto& gamma,
    const auto& theNode,
...
[[nodiscard]] constexpr bool process_recharge_count(
    double& new_resource, const double initial_resource, const auto& theNode,
    const auto& options) {  // Added options parameter
...
bool BucketGraph::process_all_resources(
    ...
    return std::all_of(new_resources.begin(), new_resources.begin() + N,
                       [&, resource_idx = size_t(0)](double& resource) mutable {
                           return this->process_resource<D>(
                               resource, initial_resources, gamma, theNode,
                               resource_idx++);
                       });
}
...
constexpr bool BucketGraph::process_resource(
    double& new_resource, const std::array<double, R_SIZE>& initial_resources,
    const Gamma& gamma, const VRPNode& theNode, const size_t I) {
    switch (this->options.resource_type[I]) {
        case DISPOSABLE:
            [[likely]] return process_disposable_resource<D>(
                new_resource, initial_resources[I], gamma.resource_increment[I],
                theNode, I);

        case NON_DISPOSABLE:
            [[likely]] return process_non_disposable_resource<D>(
                new_resource, initial_resources[I], gamma.resource_increment[I],
                theNode, I);

        case BINARY:
            [[unlikely]] return process_binary_resource<D>(
                new_resource, gamma.resource_increment[I]);

        case MTW:
            [[unlikely]] return process_mtw_resource<D>(
                new_resource, initial_resources[I], gamma.resource_increment[I],
                theNode, I);

        case BATTERY_RESOURCE:
            [[unlikely]] return process_battery_resource<D>(
                new_resource, initial_resources[I], gamma, theNode,
                this->options);

        case RECHARGE_COUNT:
            [[unlikely]] return process_recharge_count<D>(
                new_resource, initial_resources[I], theNode, this->options);

        case RECHARGE_TIME:
            [[unlikely]] return process_recharge_time<D>(
                new_resource, initial_resources[I], gamma, theNode,
                this->options);

        default:
            [[unlikely]] return false;  // Invalid resource type
    }
}
