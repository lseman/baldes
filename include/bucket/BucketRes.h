...
inline bool process_disposable_resource(double &new_resource, double initial_resource, double increment, double lb,
                                        double ub) {
...
template <Direction D>
inline bool process_non_disposable_resource(double &new_resource, double initial_resource, double increment, double lb,
                                            double ub) {
...
inline double process_binary_resource(double increment) {
    ...
    return (increment > 0) ? (D == Direction::Forward ? 1.0 : 0.0) : (D == Direction::Forward ? 0.0 : 1.0);
}

template <Direction D>
inline bool process_battery_resource(double &new_resource, double initial_resource, bool is_station,
    ...
    return D == Direction::Forward ? new_resource >= 0 : new_resource <= battery_capacity;
}

template <Direction D>
inline bool process_recharge_count(double &new_resource, double initial_resource, bool is_station,
...
template <Direction D>
inline bool process_recharge_time(double &new_resource, double initial_resource, bool is_station,
                                  double max_recharge_time) {
    new_resource = is_station ? 0 : initial_resource;
    return new_resource <= max_recharge_time;
}

template <Direction D>
inline bool process_mtw_resource(double &new_resource, double initial_resource, double increment,
...
bool BucketGraph::process_all_resources(std::vector<double>              &new_resources,
    ...
    for (size_t I = 0; I < N; ++I) {
        ...
        [[likely]] case ResourceType::Disposable:
            if (!process_disposable_resource<D>(new_resources[I], initial_resources[I], gamma.resource_increment[I],
                                                theNode.lb[I], theNode.ub[I])) {
                return false;
            }
            break;

        case ResourceType::NonDisposable:
            if (!process_non_disposable_resource<D>(new_resources[I], initial_resources[I], gamma.resource_increment[I],
                                                    theNode.lb[I], theNode.ub[I])) {
                return false;
            }
            break;

        case ResourceType::Binary: new_resources[I] = process_binary_resource<D>(gamma.resource_increment[I]); break;

        case ResourceType::Battery:
            if (!process_battery_resource<D>(new_resources[I], initial_resources[I], theNode.is_station,
                                             options.battery_capacity)) {
                return false;
            }
            break;

        case ResourceType::RechargeCount:
            if (!process_recharge_count<D>(new_resources[I], initial_resources[I], theNode.is_station,
                                           options.max_recharges)) {
                return false;
            }
            break;

        case ResourceType::RechargeTime:
            if (!process_recharge_time<D>(new_resources[I], initial_resources[I], theNode.is_station,
                                          options.max_recharge_time)) {
                return false;
            }
            break;

        case ResourceType::MTW:
            if (!process_mtw_resource<D>(new_resources[I], initial_resources[I], gamma.resource_increment[I],
                                         theNode)) {
                return false;
            }
            break;
