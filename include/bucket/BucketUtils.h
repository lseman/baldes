...
void BucketGraph::define_buckets() {
    ...
    for (const auto &VRPNode : nodes) {
        ...
        // Check if there is only one interval and proceed with the single
        // interval logic
        if (num_intervals == 1) {
            for (auto j = 0; j < intervals[0].interval; ++j) {
                ...
                if constexpr (D == Direction::Forward) {
                    interval_start[0] =
                        VRPNode.lb[0] + current_pos[0] * node_base_interval[0];
                    ...
                    if (j == intervals[0].interval - 1) {
                        ...
                        interval_end[0] =
                            VRPNode.lb[0] +
                            (current_pos[0] + 1) * node_base_interval[0];
                    }
                } else {
                    if (j == intervals[0].interval - 1) {
                    ...
                    interval_end[0] =
                        VRPNode.ub[0] - current_pos[0] * node_base_interval[0];
                }
            ...
            // Multiple intervals case, nested loop logic to generate all
            // combinations
            ...
            while (true) {
                for (int r = 0; r < num_intervals; ++r) {
                    if constexpr (D == Direction::Forward) {
                        interval_start[r] =
                            VRPNode.lb[r] + pos[r] * node_base_interval[r];
                        ...
                        interval_end[r] =
                            VRPNode.ub[r] - pos[r] * node_base_interval[r];
                    }

                    interval_start[r] =
                        roundToTwoDecimalPlaces(interval_start[r]);
                ...
                /*
                                ...
                                for (int i = 0; i < num_intervals; ++i) {
                                    fmt::print("({}, {})", interval_start[i],
                   interval_end[i]); if (i < num_intervals - 1) fmt::print(",
                   ");
                                }
                                fmt::print("]\n");
                */
...
void BucketGraph::generate_arcs() {
    ...
    // Assign the forward or backward fixed buckets and other bucket-related
    // structures
    ...
    auto add_arcs_for_node = [&](const VRPNode &node, int from_bucket,
        ...
        // Retrieve the arcs for the node in the given direction
        // (Forward/Backward)
        auto arcs = node.get_arcs<D>();
        // Compute base intervals for each resource dimension based on R_max and
        // R_min
        ...
        for (const auto &arc : arcs) {
            const auto &next_node =
                nodes[arc.to];  // Get the destination node of the arc

            std::vector<double> next_node_intervals(options.resources.size());
            for (int r = 0; r < options.resources.size(); ++r) {
                next_node_intervals[r] =
                    (next_node.ub[r] - next_node.lb[r]) / intervals[r].interval;
            }
            // Skip self-loops (no arc from a node to itself)
            if (node.id == next_node.id) continue;

            // Calculate travel cost and cost increment based on node's
            // properties
            ...
            for (int j = 0; j < num_buckets[next_node.id]; ++j) {
                ...
                if (is_arc_fixed<D>(from_bucket, to_bucket))
                    continue;  // Skip fixed arcs

                bool valid_arc = true;
                for (int r = 0; r < res_inc.size(); ++r) {
                    // Forward direction: Check that resource increment doesn't
                    // exceed upper bounds
                    if constexpr (D == Direction::Forward) {
                    ...
                    // Backward direction: Check that resource decrement doesn't
                    // drop below lower bounds
                    else if constexpr (D == Direction::Backward) {
                ...
                // Further refine arc validity based on the base intervals and
                // node bounds
                if constexpr (D == Direction::Forward) {
                    for (int r = 0; r < res_inc.size(); ++r) {
                        double max_calc =
                ...
                // Store the arc data locally before committing it to the global
                // structure
                ...
                // Add the arc to the global structure and the bucket
                {
                    std::lock_guard<std::mutex> lock(
                        buckets_mutex);  // Lock the mutex to ensure thread
                                         // safety
                    add_arc<D>(from_bucket, to_bucket, local_res_inc,
                               local_cost_inc);  // Add the arc globally
                    buckets[from_bucket].template add_bucket_arc<D>(
                        from_bucket, to_bucket, local_res_inc, local_cost_inc,
        ...
        [this, &tasks, &num_buckets, &num_buckets_index, &add_arcs_for_node,
         chunk_size](std::size_t chunk_idx) {
            ...
            for (size_t task_idx = start_idx; task_idx < end_idx; ++task_idx) {
                ...
                std::vector<double> res_inc(
                    options.resources.size());  // Resource increment vector
                std::vector<std::pair<int, int>>
                    local_arcs;  // Local storage for arcs

                // Generate arcs for all buckets associated with the current
                // node
                for (int i = 0; i < num_buckets[VRPNode.id]; ++i) {
                    int from_bucket =
                        i + num_buckets_index[VRPNode.id];  // Determine the
                                                            // source bucket
                    add_arcs_for_node(
                        VRPNode, from_bucket, res_inc,
    ...
    auto add_arcs_for_node = [&](const VRPNode &node, int from_bucket,
        ...
        for (const auto &next_node : nodes) {
            ...
            if (options.pstep == true) {
                if (next_node.id == options.pstep_depot ||
            ...
            for (int r = 0; r < options.resources.size(); ++r) {
                if (options.resources[r] == "time") {
                    if constexpr (SYM == Symmetry::Asymmetric) {
                        res_inc[r] =
                            travel_cost +
                            node.duration;  // Update resource increment based
                                            // on node duration
                    } else {
                        res_inc[r] = node.duration / 2 + travel_cost +
                                     next_node.duration /
                                         2;  // Update resource increment based
                                             // on node duration
                    }
                } else {
                    if constexpr (SYM == Symmetry::Asymmetric) {
                        ...
                        res_inc[r] = node.consumption[r] / 2 +
                                     next_node.consumption[r] / 2;
                    }
                }
            }
