...
private:
    ...
    double calculateAverageDifference(const std::vector<double> &nodeDuals) const {
        if (prevDuals.empty()) return 0.0;

        double differenceSum = 0.0;
        for (size_t i = 0; i < numConstrs; i++) { differenceSum += std::abs(nodeDuals[i] - prevDuals[i]); }
    ...
    double calculateAdaptiveRate() const {
        if (std::abs(state.prev_inner_obj) < Parameters::EPSILON) { return Parameters::CURVATURE_RATE; }
        return std::min(Parameters::CURVATURE_RATE * (1.0 + std::abs(state.prev_inner_obj - state.inner_obj) /
    ...
    void updateDiffHistory(double currentDiff) {
        ...
        if (diffHistory.size() > Parameters::HISTORY_SIZE) { diffHistory.erase(diffHistory.begin()); }
    }
    ...
    void updateInnerInterval(double avgDiff) {
        if (state.savedInnerInterval == 0.0) {
            ...
            state.savedInnerInterval += (avgDiff - state.savedInnerInterval) * alpha;
        }
    }

    void updateTrustRegionSize() {
        if (state.savedInnerInterval < std::numeric_limits<double>::infinity()) {
            state.innerHalfInterval = state.savedInnerInterval * calculateAdaptiveHalfInterval();
            state.v                 = state.savedInnerInterval / (state.c * calculateTRScaling());
            state.v                 = std::clamp(state.v, Parameters::MIN_V, Parameters::MAX_V);
        } else {
    ...
    void updateVariables(const std::vector<double> &nodeDuals, double new_eps) {
        for (int i = 0; i < numConstrs; i++) {
            double safeDual = std::clamp(nodeDuals[i], -Parameters::MAX_DUAL_CHANGE, Parameters::MAX_DUAL_CHANGE);

            w[i]->setUB(new_eps);
            zeta[i]->setUB(new_eps);

            w[i]->setOBJ(std::max(0.0, safeDual - std::clamp(state.v, 0.0, Parameters::MAX_V)));
            zeta[i]->setOBJ(safeDual + std::clamp(state.v, 0.0, Parameters::MAX_V));
        }
    }
...
public:
    ...
    void setup(BNBNode *node, const std::vector<double> &nodeDuals) {
        state.v = std::accumulate(nodeDuals.begin(), nodeDuals.end(), 0.0) / numConstrs;
        state.c = state.v / state.kappa;

        for (int i = 0; i < numConstrs; i++) {
            w[i]    = node->addVar("w_" + std::to_string(i + 1), VarType::Continuous, 0, state.epsilon1, -1.0);
            zeta[i] = node->addVar("zeta_" + std::to_string(i + 1), VarType::Continuous, 0, state.epsilon2, 1.0);
    ...
    double iterate(BNBNode *node, std::vector<double> &nodeDuals, double inner_obj, int stage, bool transition) {
        state.isInsideTrustRegion = true;
        state.TRstop              = false;
        state.inner_obj           = inner_obj;

        state.isInsideTrustRegion = std::all_of(nodeDuals.begin(), nodeDuals.end(), [this, i = 0](double dual) mutable {
            return dual >= delta1[i] && dual <= delta2[i++];
        });

        double dualDiffSum = std::transform_reduce(nodeDuals.begin(), nodeDuals.end(), prevDuals.begin(), 0.0,
        ...
        if (state.isInsideTrustRegion && dualDiffSum > Parameters::EPSILON) {
            ...
            state.v = avg_diff < state.v ? std::max(state.v * 0.9, avg_diff) : std::min(state.v * 1.1, avg_diff);

            double progress = (inner_obj - state.prev_inner_obj) / std::abs(state.prev_inner_obj + Parameters::EPSILON);
            adjustParameters(progress);

            double new_eps = std::clamp(state.v / state.c, Parameters::MIN_V, Parameters::MAX_EPSILON);
