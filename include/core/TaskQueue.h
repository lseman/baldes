...
public:
    ...
    TaskQueue(int threshold, exec::static_thread_pool::scheduler sched, MotherClass &mother)
        ...
        if (threshold <= 0) { throw std::invalid_argument("Task threshold must be positive"); }
...
private:
    ...
    exec::task<void> wait_for_tasks() {
        ...
        queue_condition.wait(lock, [this] {
            return shutdown || (task_queue.size() >= static_cast<size_t>(task_threshold)) || !task_queue.empty();
        });
        co_return;
    }
    ...
    exec::task<void> process_tasks() {
        ...
        if (shutdown && task_queue.empty()) { co_return; }

        // Use a local vector for batch processing.
        std::vector<TaskType> tasks_to_process;
        {
            std::scoped_lock lock(queue_mutex);
            const size_t     batch_size = std::min(static_cast<size_t>(task_threshold), task_queue.size());
        ...
        if (tasks_to_process.empty()) { co_return; }
        ...
        if (!result.empty()) {
            ...
            if (processed_tasks.size() > processed_tasks.max_size() - result.size()) {
                throw std::runtime_error("Exceeding maximum vector size");
            }
            if (processed_tasks.size() + result.size() > MAX_PROCESSED_TASKS) {
                throw std::runtime_error("Exceeding maximum allowed processed tasks");
            }
    ...
    exec::task<void> task_worker() {
        ...
        } catch (const std::exception &e) { std::cerr << "Error in task worker: " << e.what() << std::endl; }
        co_return;
    }
