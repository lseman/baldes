...
public:
    ...
    baldesCtr(const LinearExpression &expr, double rhs, char relation)
        : expression(expr), rhs(rhs), relation(relation), unique_id(generate_unique_id()),
          indice(-1) // Initialize index to invalid value
    {}

    // Default constructor
    baldesCtr()
        : expression(), rhs(0.0), relation('='), unique_id(generate_unique_id()),
          indice(-1) // Initialize index to invalid value
    {}

    // Copy constructor needs to generate a new unique_id
    baldesCtr(const baldesCtr &other)
        : expression(other.expression), rhs(other.rhs), relation(other.relation), unique_id(other.unique_id),
          indice(other.indice), name(other.name) {}

    baldesCtr(const LinearExpression &expr, double rhs, char relation, size_t existing_id, int indice,
              const std::string &name = "")
        : expression(expr), rhs(rhs), relation(relation), unique_id(existing_id) // Use existing ID
          ,
          indice(indice), name(name) {}

    std::shared_ptr<baldesCtr> clone() const {
        // Use the private constructor to create a clone with same unique_id
        return std::shared_ptr<baldesCtr>(new baldesCtr(expression, rhs, relation, unique_id, indice, name));
    }
