    ...
    void allocate_labels(size_t count) {
        ...
        for (size_t b = 0; b < blocks_needed; ++b) {
            ...
            for (size_t i = 0; i < alloc_in_block; ++i) {
                free_list.push_back(base_ptr + i);
            }
...
public:
    ...
    inline void reset() noexcept {
        ...
        if (free_list.size() < total_used) {
            free_list.resize(total_used);
        }
        ...
        for (auto &block : memory_blocks) {
            ...
            for (size_t i = 0; i < block.used; ++i) {
                free_list[free_list_size++] = base_ptr + i;
            }
        }
    ...
    // Get statistics
    [[nodiscard]] inline size_t get_free_count() const noexcept {
    ...
    void shrink_to_fit() {
        ...
        while (!memory_blocks.empty() && memory_blocks.back().used == 0) {
