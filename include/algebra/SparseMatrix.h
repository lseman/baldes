...
struct SparseMatrix {
    ...
    SparseMatrix(const SparseMatrix &other)
        : rows(other.rows), cols(other.cols), values(other.values), crs_data(other.crs_data), coo_mode(other.coo_mode),
    ...
    void copyCRSToCOO(const SparseMatrix::CRSData &crs) {
        ...
        for (int row = 0; row < num_rows; ++row) {
            ...
            int row_end   = (row + 1 < num_rows) ? crs.row_start[row + 1] : static_cast<int>(nnz);
    ...
    public:
        RowIterator(std::span<const int> cols_, std::span<const double> values_, size_t start, size_t end)
            : cols(cols_), values(values_), current_index(start), end_index(end) {}
        ...
        value_type operator*() const {
            if (current_index >= end_index) throw std::out_of_range("Iterator out of range");
    ...
    RowIterator row_begin(int row) const {
        if (coo_mode) throw std::runtime_error("CRS data not available in COO mode.");
        const auto &crs = *crs_data;
        if (row < 0 || row >= num_rows) throw std::out_of_range("Row index out of bounds");
        size_t start = crs.row_start[row];
        size_t end   = (row + 1 < num_rows) ? crs.row_start[row + 1] : crs.values.size();
    ...
    std::vector<double> get_row_values(int row) const {
        ...
        for (auto it = row_begin(row); it != row_end(row); ++it) { result.push_back((*it).second); }
        return result;
    }
    ...
    std::vector<double> multiply(const std::vector<double> &x) const {
        ...
        for (int row = 0; row < num_rows; ++row) {
            int    row_start = crs.row_start[row];
            int    row_end   = (row + 1 < num_rows) ? crs.row_start[row + 1] : crs.values.size();
            double sum       = 0.0;
    ...
    void modify_or_delete_batch(const std::vector<int> &r, const std::vector<int> &c, const std::vector<double> &vals) {
        if (r.size() != c.size() || c.size() != vals.size())
            throw std::invalid_argument("Input vectors must have the same length.");
        if (!coo_mode) switchToCOO();
        for (size_t i = 0; i < vals.size(); ++i) { modify_or_delete(r[i], c[i], vals[i]); }
    }
    ...
    std::vector<int> &getIndices() {
        if (coo_mode) throw std::runtime_error("Indices only available in CRS mode.");
        return crs_data->cols;
    }
    std::vector<double> &getValues() {
        if (coo_mode) throw std::runtime_error("Values only available in CRS mode.");
