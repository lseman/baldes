...
class Supernodal {
public:
    using MatrixType = Eigen::SparseMatrix<Scalar, Eigen::ColMajor, StorageIndex>;
    using VectorType = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;

    Supernodal(const MatrixType &matrix) : m_matrix(matrix) {
        if (m_matrix.rows() != m_matrix.cols()) { throw std::runtime_error("Matrix must be square"); }
    ...
    void printEliminationTree() const {
        ...
        for (StorageIndex i = 0; i < n; i++) {
            std::cout << "Node " << i << " -> Parent: " << m_parent[i] << std::endl;
        }
        ...
        for (StorageIndex i = 0; i < n; i++) {
            if (is_root[i]) { printNode(i, children, "", ""); }
        }
    }

    void debugSupernodes() const {
        std::cout << "\nSupernode Analysis:" << std::endl;
        std::cout << "Number of supernodes: " << m_supernodes.size() << std::endl;

        for (size_t i = 0; i < m_supernodes.size(); ++i) {
            const auto &supernode = m_supernodes[i];
            std::cout << "Supernode " << i << ": size = " << supernode.size() << ", columns = ";
            ...
            for (typename MatrixType::InnerIterator it(m_matrix, first_col); it; ++it) {
                if (it.row() >= first_col) { std::cout << it.row() << " "; }
            }
            std::cout << std::endl;
        }
    }
    VectorType solve(const VectorType &b) {
        if (b.size() != m_matrix.rows()) { throw std::runtime_error("Right-hand side size mismatch"); }
...
private:
    void printNode(StorageIndex node, const std::vector<std::vector<StorageIndex>> &children, const std::string &prefix,
        ...
        for (size_t i = 0; i < children[node].size(); ++i) {
            ...
            std::string newChildPrefix = childPrefix + (isLast ? "    " : "│   ");
    ...
    void computeSupernodes() {
        ...
        for (StorageIndex col = 0; col < n; col++) {
            for (typename MatrixType::InnerIterator it(m_matrix, col); it; ++it) {
        ...
        while (current_start < n) {
            StorageIndex              supernode_size = std::min(bandwidth + 1, n - current_start);
    ...
    void factorSupernode(StorageIndex first_col, StorageIndex size, const std::vector<StorageIndex> &supernode,
                         const std::vector<StorageIndex> &map_to_packed,
                         const std::vector<StorageIndex> &idx_to_pattern, StorageIndex pattern_size,
        ...
        for (StorageIndex j = 0; j < size; j++) {
            ...
            for (StorageIndex i = j; i < size; i++) {
                if (std::abs(L(i, j)) > 1e-14) {
                    ...
                    std::cout << "Store L(" << supernode[i] << "," << supernode[j] << ") = " << L(i, j) << "\n";
                }
            }
        }
        ...
        if (pattern_size > size) {
            ...
            for (StorageIndex j = 0; j < size; j++) {
                // Scale by D inverse
                if (std::abs(D(j)) > 1e-14) { L21.col(j) /= D(j); }
                std::cout << "After D scaling for col " << j << ":\n" << L21 << "\n";

                // Update remaining columns
                for (StorageIndex k = j + 1; k < size; k++) { L21.col(k) -= L21.col(j) * L(k, j); }
            ...
            for (StorageIndex i = 0; i < pattern_size - size; i++) {
                ...
                for (StorageIndex j = 0; j < size; j++) {
                    if (std::abs(L21(i, j)) > 1e-14) {
                        ...
                        std::cout << "Store L(" << row << "," << supernode[j] << ") = " << L21(i, j) << "\n";
                    }
                }
            }
        }
    ...
    void computeLDLTFactorization() {
        ...
        for (size_t snode_idx = 0; snode_idx < m_supernodes.size(); snode_idx++) {
            const auto  &supernode = m_supernodes[snode_idx];
            StorageIndex size      = supernode.size();
            StorageIndex first_col = supernode[0];

            std::cout << "\n=== Processing Supernode " << snode_idx << " (cols " << first_col << " to "
                      << first_col + size - 1 << ") ===\n";
            ...
            for (StorageIndex k = 0; k < first_col; k++) {
                ...
                if (has_update) {
                    ...
                    for (StorageIndex i = 0; i < size; i++) { L_curr(i) = m_L.coeff(supernode[i], k); }
                    ...
                    if (std::abs(m_D[k]) > 1e-14) {
                        Eigen::MatrixXd update = (L_col * L_curr.transpose()) * m_D[k];
            ...
            // Factor supernode
            factorSupernode(first_col, size, supernode, map_to_packed, idx_to_pattern, pattern_size, dense);
    ...
    void solveInternal(VectorType &x) const {
        ...
        for (StorageIndex i = 0; i < n; i++) {
            if (std::abs(m_D[i]) < 1e-14) { throw std::runtime_error("Near-zero diagonal encountered"); }
            x(i) /= m_D[i];
        }
        ...
        for (StorageIndex j = n - 1; j >= 0; j--) {
            ...
            Scalar sum = 0;
            for (typename MatrixType::InnerIterator it(m_L, j); it; ++it) {
                if (it.row() > j) { sum += it.value() * x(it.row()); }
            }
            x(j) -= sum;
        ...
        std::cout << "Scaled residual |b - Ax|/(|A|*|x|): " << r_norm / (A_norm * x_norm) << "\n";
        ...
        std::cout << "\nResidual components:\n";
        for (StorageIndex i = 0; i < n; i++) { std::cout << "r[" << i << "] = " << r(i) << "\n"; }
    }
    ...
    void checkFactorizationAccuracy() const {
        ...
        for (StorageIndex j = 0; j < n; j++) {
            ...
            for (typename MatrixType::InnerIterator it(LD, j); it; ++it) { it.valueRef() *= m_D[j]; }
        }
        ...
        for (StorageIndex j = 0; j < n; j++) {
            for (typename MatrixType::InnerIterator it1(LD, j); it1; ++it1) {
                ...
                for (typename MatrixType::InnerIterator it2(m_L, j); it2; ++it2) {
        ...
        for (StorageIndex j = 0; j < n; j++) {
            for (typename MatrixType::InnerIterator it(m_matrix, j); it; ++it) {
                ...
                if (diff > 1e-10) {
                    std::cout << "Large difference at (" << it.row() << "," << j << "): original=" << orig
                              << ", reconstructed=" << recon << ", diff=" << diff << "\n";
                }
            }
        }
        std::cout << "Maximum difference between original and reconstructed: " << max_diff << "\n";
    }
