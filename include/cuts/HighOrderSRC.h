   ...
   public:
    ...
    void startSeedCrazy() {
        ...
        WorkStealingPool pool(
            max_threads);        // Use a thread pool for parallel processing
                        ...
                        if (best_vio > initial_vio) {
...

                            {
                                ...
                                rank1_multi_label_pool[num_label++] =
                                    Rank1MultiLabel(
                                        std::move(new_c), std::move(new_w_no_c),
                                        plan_idx, best_vio, best_op);
                            }
                        }
...
#pragma omp simd
    ...
    void operationsCrazy(Rank1MultiLabel &label, int &i) {
        ...
        std::visit(
            overloaded{
                [](const std::monostate &) { /* No operation */ },
                [&](const int j) {
                    if (best_idx == 1) {  // Add
                        label.w_no_c.erase(std::remove(label.w_no_c.begin(),
                                                       label.w_no_c.end(), j),
                                           label.w_no_c.end());
                        label.c.push_back(j);
                    } else {  // Remove
                        label.c.erase(
                            std::remove(label.c.begin(), label.c.end(), j),
                            label.c.end());
                    }
                },
                [&](const std::pair<int, int> &swap_pair) {
                    ...
                    if (auto it =
                            std::find(label.c.begin(), label.c.end(), from);
                        it != label.c.end()) {
                        *it = to;
                        label.w_no_c.erase(std::remove(label.w_no_c.begin(),
                                                       label.w_no_c.end(), to),
