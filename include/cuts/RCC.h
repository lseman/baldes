/**
 * @file RCC.h
 * @brief Definitions for generating and separating Rounded Capacity Cuts (RCC)
 * in the context of vehicle routing problems.
 *
 * This header file contains the structure and function definitions required for
 * separating Rounded Capacity Cuts (RCCs) using Gurobi's optimization model.
 * RCC separation is an important aspect of optimization algorithms,
 * particularly in vehicle routing problems where capacity constraints must be
 * enforced.
 *
...
class RCCManager {
   public:
    ...
    std::vector<double> computeRCCCoefficients(
        const std::vector<uint16_t> &label) {
        ...
        for (int j = 0; j < label.size() - 1; j++) {
            label_arcs.insert({label[j], label[j + 1]});
        }
        ...
        for (int i = 0, n_cuts = cuts_.size(); i < n_cuts; i++) {
            ...
            // Iterate over arcs in the cut and check if they exist in the
            // precomputed label arcs
            for (const auto &arc : cuts_[i].arcs) {
                coeff += label_arcs.count({arc.from, arc.to});
            }
    ...
    // Compute the dual values for each arc by summing the duals of cuts passing
    // through the arc
    ...
    ArcDuals computeDuals(std::vector<double> dualValues, BNBNode *node,
