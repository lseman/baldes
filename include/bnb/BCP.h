...
*/
            ...
            while (misprice) {
                ...
                bucket_graph->augment_ng_memories(solution, allPaths, true,
                                                  eta1, eta2, eta_max, N_SIZE);
                ...
                // auto pseudo_bound = stab.compute_pseudo_dual_bound(matrix,
                // nodeDuals, paths);
                lag_gap = integer_solution - (lp_obj + numK * inner_obj);
                auto d = 50;
#ifdef HIGHS
                gap = std::abs(lp_obj - (lp_obj + std::min(0.0, inner_obj))) /
                      std::abs(lp_obj);
                gap = gap / d;
                if (std::isnan(gap)) {
                    gap = 1e-2;
                ...
                //     if (aggressive) {
                //         aggressive = false; // Switch to less aggressive
                //         forbidding
                //     } else {
                ...
                // } else {
                //     // Solution improved
                //     eta1    = std::max(5, eta1 - 1);     // Tighten cycle
                //     size limit eta2    = std::max(110, eta2 - 10);  // Reduce
                //     number of cycles to forbid eta_max = std::max(18, eta_max
                //     - 1); // Tighten neighborhood size
                // }
