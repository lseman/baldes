/**
 * @file
 * @brief This file contains the implementation of the Minimum Spanning Tree (MST) algorithm for clustering.
 *
 * This file contains the implementation of the Minimum Spanning Tree (MST) algorithm for clustering.
 * The MST algorithm is used to find the minimum spanning tree of a graph and then cluster the nodes
 * based on the edges of the MST. The file includes the implementation of the MST class, which computes
...
public:
    ...
    // Function to perform clustering by removing edges with costs exceeding threshold Θ
    std::vector<std::vector<int>> cluster(double theta) {
        ...
        double avg = std::accumulate(mst.begin(), mst.end(), 0.0,
                                     [](double sum, const Arc &arc) { return sum + std::get<0>(arc); }) /
                     mst.size();

        double std_dev = std::sqrt(
            std::accumulate(mst.begin(), mst.end(), 0.0,
                            [avg](double sum, const Arc &arc) { return sum + std::pow(std::get<0>(arc) - avg, 2); }) /
            mst.size());
        ...
        std::copy_if(mst.begin(), mst.end(), std::back_inserter(filtered_edges),
                     [threshold](const Arc &arc) { return std::get<0>(arc) <= threshold; });
...
private:
    ...
    // Helper function to find connected components (clusters) from the filtered edges
