...
concept Trivially_Serializable =
    std::is_trivially_copyable_v<T> && !Array_Like<T> && !Container<T> && !String_Like<T> && !Reflectable<T>;
...
#define FOR_EACH(macro, ...)                                                                                \
    GET_FOR_EACH_MACRO(__VA_ARGS__, FOR_EACH_7, FOR_EACH_6, FOR_EACH_5, FOR_EACH_4, FOR_EACH_3, FOR_EACH_2, \
                       FOR_EACH_1)                                                                          \
    (macro, __VA_ARGS__)
...
template <Array_Like T>
void serialize_value(const T &arr, std::ostream &os) {
    for (const auto &item : arr) { serialize_value(item, os); }
}

template <Array_Like T>
void deserialize_value(T &arr, std::istream &is) {
    for (auto &item : arr) { deserialize_value(item, is); }
}
...
void deserialize_value(T &str, std::istream &is) {
    ...
    str.resize(size);
    if (!is.read(&str[0], size)) { throw SerializationError("Failed to read string data"); }
}
...
void serialize_value(const T &container, std::ostream &os) {
    ...
    serialize_value(size, os);
    for (const auto &item : container) { serialize_value(item, os); }
}
...
void deserialize_value(T &container, std::istream &is) {
    ...
    deserialize_value(size, is);
    container.clear();
    if constexpr (requires { container.reserve(size); }) { container.reserve(size); }
...
public:
    static void toFile(const T &obj, const std::string &filename) {
        ...
        if (!ofs) { throw SerializationError("Could not open file for writing: " + filename); }
        serialize(obj, ofs);
    }

    static void fromFile(T &obj, const std::string &filename) {
        std::ifstream ifs(filename, std::ios::binary);
        if (!ifs) { throw SerializationError("Could not open file for reading: " + filename); }
        deserialize(obj, ifs);
    }
};
