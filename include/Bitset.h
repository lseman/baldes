/*
 * @file Bitset.h
 * @brief Defines the Bitset utility for bit-level operations.
 *
 */
#pragma once
...
public:
    ...
    [[nodiscard]] constexpr auto span() const noexcept { return std::span(bits_); }
...
struct hash<Bitset<N_BITS>> {
    [[nodiscard]] size_t operator()(const Bitset<N_BITS> &bitset) const noexcept {
        return XXH3_64bits_withSeed(bitset.bits_.data(), bitset.bits_.size() * sizeof(uint64_t),
