/*
 * @file MIPHelper.h
 * @brief Declares MIPHelper interfaces and types used by the BALDES solver.
 *
 * This file declares the MIPHelper interfaces and helper functions used by the BALDES solver.
 *
 */
...
public:
    ...
    std::pair<int, double> get_term(size_t index) const {
        if (index >= terms.size()) { throw std::out_of_range("Index out of range"); }
        return terms[index];
    }
...
public:
    explicit GurobiModelCache(GRBEnv &env) { model = std::make_unique<GRBModel>(env); }

    // Copy constructor
    GurobiModelCache(const GurobiModelCache &other)
        : pendingChanges(other.pendingChanges), gurobiVars(other.gurobiVars), model(new GRBModel(*other.model)),
    ...
    void initialize(const std::vector<baldesVarPtr> &variables, const std::vector<baldesCtrPtr> &constraints,
        ...
        try {
            ...
            for (const auto &var : variables) {
                gurobiVars.emplace(var->get_name(),
                                   model->addVar(var->get_lb(), var->get_ub(), var->get_objective_coefficient(),
            ...
            for (const auto &constraint : constraints) {
                ...
                case '<': model->addConstr(expr <= constraint->get_rhs(), constraint->get_name()); break;
                case '>': model->addConstr(expr >= constraint->get_rhs(), constraint->get_name()); break;
                case '=': model->addConstr(expr == constraint->get_rhs(), constraint->get_name()); break;
                }
            }
            ...
            model->setObjective(objective, isMinimize ? GRB_MINIMIZE : GRB_MAXIMIZE);
            model->update();
    ...
    void printCacheContents() const {
        ...
        for (size_t i = 0; i < pendingChanges.size(); ++i) {
            ...
            case ChangeType::AddVariable:
                fmt::print("Add Variable: {} (lb={}, ub={}, obj_coeff={})\n", change.var->get_name(),
                           change.var->get_lb(), change.var->get_ub(), change.var->get_objective_coefficient());
                break;

            case ChangeType::DeleteVariable: fmt::print("Delete Variable at index {}\n", change.col); break;

            case ChangeType::AddConstraint:
                fmt::print("Add Constraint: {} ({} {})\n", change.ctr->get_name(), change.ctr->get_relation(),
                           change.ctr->get_rhs());
                change.ctr->get_expression().print_expression();
                break;

            case ChangeType::DeleteConstraint: fmt::print("Delete Constraint at index {}\n", change.row); break;

            case ChangeType::ModifyCoefficient:
                fmt::print("Modify Coefficient: row={}, col={}, new_value={}\n", change.row, change.col, change.value);
                break;
            }
        }
        ...
        for (const auto &[name, var] : gurobiVars) {
            try {
                fmt::print("  {}: type={}, lb={}, ub={}, obj_coeff={}\n", name, var.get(GRB_CharAttr_VType),
                           var.get(GRB_DoubleAttr_LB), var.get(GRB_DoubleAttr_UB), var.get(GRB_DoubleAttr_Obj));
            } catch (const GRBException &e) { fmt::print("  {}: <error reading variable properties>\n", name); }
        }
        ...
        try {
            ...
            fmt::print("  ModelSense: {}\n",
                       model->get(GRB_IntAttr_ModelSense) == GRB_MINIMIZE ? "Minimize" : "Maximize");
        } catch (const GRBException &e) { fmt::print("\nError reading model stats: {}\n", e.getMessage()); }
    ...
    void deleteConstraint(int ctrIndex) {
        ModelChange change{.type = ChangeType::DeleteConstraint, .row = ctrIndex};
        pendingChanges.push_back(std::move(change));
        needsRebuild = true;
    }

    void modifyCoefficient(int row, int col, double newValue) {
        ModelChange change{.type = ChangeType::ModifyCoefficient, .row = row, .col = col, .value = newValue};
        pendingChanges.push_back(std::move(change));
    }

    void modifyConstraint(int row, const std::vector<double> &values) {
        for (size_t i = 0; i < values.size(); ++i) { modifyCoefficient(row, i, values[i]); }
    }

    GRBModel *getModel() {
        // print pendingChanges status
        if (!pendingChanges.empty()) { applyChanges(); }
        return model.get();
    }
        ...
        try {
            for (const auto &change : pendingChanges) {
                ...
                case ChangeType::AddVariable: {
                    ...
                    gurobiVars.emplace(change.var->get_name(),                                // Variable name
                                       model->addVar(change.var->get_lb(),                    // Lower bound
                                                     change.var->get_ub(),                    // Upper bound
                                                     change.var->get_objective_coefficient(), // Objective
                                                                                              // coefficient
                                                     toGRBVarType(change.var->get_type()),    // Variable type
                                                                                              // (binary, integer,
                                                                                              // continuous)
                ...
                case ChangeType::AddConstraint: {
                    GRBLinExpr expr = 0.0;
                    for (const auto &[varName, coeff] : change.ctr->get_terms()) {
                        expr += gurobiVars.at(varName) * coeff;
                    }

                    switch (change.ctr->get_relation()) {
                    case '<': model->addConstr(expr <= change.ctr->get_rhs(), change.ctr->get_name()); break;
                    case '>': model->addConstr(expr >= change.ctr->get_rhs(), change.ctr->get_name()); break;
                    case '=': model->addConstr(expr == change.ctr->get_rhs(), change.ctr->get_name()); break;
                    }
                    break;
                ...
                case ChangeType::DeleteVariable: model->remove(model->getVar(change.col)); break;

                case ChangeType::DeleteConstraint: model->remove(model->getConstr(change.row)); break;

                case ChangeType::ModifyCoefficient:
                    model->chgCoeff(model->getConstr(change.row), model->getVar(change.col), change.value);
                    break;
                }
            }
