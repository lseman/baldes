...
public:
...
#ifdef GUROBI
        gurobiCache = std::make_unique<GurobiModelCache>(GurobiEnvSingleton::getInstance());
#endif
    }

    // Copy constructor
    MIPProblem(const MIPProblem &other)
        : name(other.name), variables(other.variables), constraints(other.constraints), objective(other.objective),
          objective_type(other.objective_type), sparse_matrix(other.sparse_matrix),
          var_name_to_index(other.var_name_to_index), b_vec(other.b_vec) {
#ifdef GUROBI
    ...
    MIPProblem(MIPProblem &&other) noexcept
        : name(std::move(other.name)), variables(std::move(other.variables)), constraints(std::move(other.constraints)),
          objective(std::move(other.objective)), objective_type(other.objective_type),
          sparse_matrix(std::move(other.sparse_matrix)), var_name_to_index(std::move(other.var_name_to_index)),
    ...
    void chgCoeff(int constraintIndex, const std::vector<double> &values) {
        ...
        for (size_t i = 0; i < values.size(); ++i) {
            ...
            sparse_matrix.modify_or_delete(constraintIndex, static_cast<int>(i), new_value);
    ...
    void chgCoeff(int constraintIndex, int variableIndex, double value) {
        // Validate indices.
        if (constraintIndex < 0 || constraintIndex >= static_cast<int>(constraints.size()) || variableIndex < 0 ||
    ...
    std::unique_ptr<GRBModel> toCachedGurobiModel(GRBEnv &env) {
        try {
            if (!gurobi_initialized) {
                ...
                gurobiCache->initialize(variables, constraints, objective_type == ObjectiveType::Minimize);
    ...
    std::vector<char> get_vtypes() const {
        ...
        for (const auto &var : variables) {
            switch (var->get_type()) {
            case VarType::Continuous: vtypes.push_back('C'); break;
            case VarType::Integer: vtypes.push_back('I'); break;
            case VarType::Binary: vtypes.push_back('B'); break;
            }
        }
        return vtypes;
    }
    ...
    std::vector<int> getBasicVariables(GRBModel *model) {
        ...
        try {
            ...
            for (int i = 0; i < num_vars; i++) {
                ...
                if (basis_status == GRB_BASIC) { basic_variables.push_back(i); }
            }

            delete[] vars; // Clean up

        } catch (GRBException &e) {
            std::cerr << "Error getting basic variables: " << e.getMessage() << std::endl;
            throw;
        }
    ...
    std::vector<int> reduceNonBasicVariables(double keep_percentage = 0.7) {
        ...
        try {
            if (model->get(GRB_IntAttr_Status) != GRB_OPTIMAL) {
                throw std::runtime_error("Model must be solved to optimality first");
            }
            ...
            for (int i = 0; i < num_vars; i++) {
                ...
                if (basis_status == GRB_BASIC) { basic_vars.push_back(i); }
            }

            pdqsort(rc_pairs.begin(), rc_pairs.end(), [](const auto &a, const auto &b) { return a.first > b.first; });

            // print the first 10 rc_pairs
            for (int i = 0; i < 10 && i < rc_pairs.size(); i++) {
                fmt::print("RC: {}, Var: {}\n", rc_pairs[i].first, rc_pairs[i].second);
            }

            // Calculate how many variables to remove
            size_t remove_count = static_cast<size_t>(std::ceil(rc_pairs.size() * (1.0 - keep_percentage)));
            ...
            for (size_t i = 0; i < remove_count && i < rc_pairs.size(); i++) {
                int var_index = rc_pairs[i].second;
                // Skip variables that are in the basic set
                if (std::find(basic_vars.begin(), basic_vars.end(), var_index) == basic_vars.end()) {
                    indices_to_remove.insert(var_index);
                }
            }

            // Convert to vector and sort in descending order
            removed_vars = std::vector<int>(indices_to_remove.begin(), indices_to_remove.end());
            std::sort(removed_vars.begin(), removed_vars.end(), std::greater<int>());

            // Delete variables
            for (int idx : removed_vars) { delete_variable(idx); }

            delete[] vars; // Clean up
            print_info("Removed {} non-basic variables with no contributing RCs\n", removed_vars.size());
