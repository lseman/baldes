...
struct Label {
    ...
    inline bool is_dominated(Label *&label) noexcept {
        // Early return if the cost condition is not met
        if (label->cost > cost) { return false; }

        const auto &label_resources = label->resources;

        if constexpr (D == Direction::Forward) {
            for (size_t i = 0; i < resources.size(); ++i) {
                if (label_resources[i] > resources[i]) { return false; }
            }
        ...
        // Optimized bitmap comparison using SIMD
        if constexpr (S == Stage::Three || S == Stage::Four || S == Stage::Enumerate) {
            for (auto i = 0; i < label->visited_bitmap.size(); ++i) {
                if ((label->visited_bitmap[i] & ~visited_bitmap[i]) != 0) {
    ...
    template <Direction D, Stage S>
    inline bool is_dominated(Label *&label) const noexcept {
        // Early return if the cost condition is not met
        if (label->cost > cost) { return false; }
    ...
    inline bool check_dominance_against_vector(const std::vector<Label *> &labels) noexcept {
        ...
        __m256d current_cost = _mm256_set1_pd(this->cost); // Assuming "this" is the current Label
                ...
                if (mask & (1 << j)) {
                    ...
                    if constexpr (D == Direction::Forward) {
                        for (size_t k = 0; k < resources.size(); ++k) {
                            ...
                            if (label_resources[k] > resources[k]) {
                                dominated = false;
                                break;
                            }
                        }
                    ...
                    if constexpr (S == Stage::Three || S == Stage::Four || S == Stage::Enumerate) {
                        const size_t bitmap_size      = this->visited_bitmap.size();
                        const size_t simd_bitmap_size = 4;
                        size_t       k                = 0;

                        for (; k + simd_bitmap_size <= bitmap_size; k += simd_bitmap_size) {
                            __m256i label_bitmap = _mm256_loadu_si256(
                                reinterpret_cast<const __m256i *>(&labels[i + j]->visited_bitmap[k]));
                            __m256i current_bitmap =
                                _mm256_loadu_si256(reinterpret_cast<const __m256i *>(&this->visited_bitmap[k]));
                        ...
                        for (; k < bitmap_size; ++k) {
                            if ((labels[i + j]->visited_bitmap[k] & ~this->visited_bitmap[k]) != 0) {
                                dominated = false;
                                break;
                            }
                        }
            ...
            if (labels[i]->cost <= this->cost) {
                ...
                if constexpr (S == Stage::Three || S == Stage::Four || S == Stage::Enumerate) {
                    ...
                    for (size_t k = 0; k < bitmap_size; ++k) {
                        if ((labels[i]->visited_bitmap[k] & ~this->visited_bitmap[k]) != 0) {
                            dominated = false;
                            break;
                        }
                    }
