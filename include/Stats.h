...
public:
    void addIteration(double obj_value, bool decision_successful = true) {
        // Validate input
        if (!std::isfinite(obj_value)) { obj_value = obj_history.empty() ? 0.0 : obj_history.back(); }
        ...
        while (obj_history.size() > MAX_HISTORY) { obj_history.pop_front(); }
        while (decision_outcomes.size() > MAX_HISTORY) { decision_outcomes.pop_front(); }
    }

    double computeThreshold(int iteration, double current_obj) {
        // Validate input
        if (!std::isfinite(current_obj)) { current_obj = obj_history.empty() ? 0.0 : obj_history.back(); }
        ...
        if (hasHistory()) {
            ...
            if (std::isfinite(improvement) && denominator > NUMERICAL_STABILITY_THRESHOLD) {
                progress_factor = std::clamp(improvement / denominator, 0.5, 1.5);
            }
        }
        ...
        double adaptive_momentum = MOMENTUM_FACTOR / (1.0 + safeValue(recent_variance));
        momentum  = safeValue(adaptive_momentum * momentum + (1.0 - adaptive_momentum) * (threshold - prev_threshold));
        ...
        if (std::uniform_real_distribution<>(0, 1)(rng) < epsilon) {
            ...
            double random_factor     = 2.0 * std::uniform_real_distribution<>(0, 1)(rng) - 1.0;
            threshold                = safeValue(threshold + random_factor * exploration_range * 0.2);
        }
...
private:
    void updateLearningRate(bool decision_successful) {
        if (hasHistory()) {
            ...
            learning_rate *= std::isfinite(rate_adjustment) ? rate_adjustment : 1.0;

            // Add cyclical component
            int          iteration       = static_cast<int>(obj_history.size());
            const double cycle_length    = 50.0;
            double       cycle_position  = std::fmod(iteration, static_cast<int>(cycle_length)) / cycle_length;
            double       cyclical_factor = 0.5 + 0.5 * std::cos(2.0 * M_PI * cycle_position);
            learning_rate *= std::isfinite(cyclical_factor) ? cyclical_factor : 1.0;

            // Bound the learning rate
            learning_rate = std::clamp(learning_rate, MIN_LEARNING_RATE, MAX_LEARNING_RATE);
        }
    }
    ...
    void updateExploration() {
        if (decision_outcomes.size() >= 20) {
            ...
            double exploration_score = safeValue((1.0 - success_rate) * std::sqrt(variance));
            epsilon                  = MIN_EPSILON + (1.0 - MIN_EPSILON) * std::exp(-exploration_score);
        }
    }
    ...
    double getRecentImprovement(size_t window) const {
        ...
        for (auto it = start + 1; it != end; ++it) {
            ...
            if (std::isfinite(prev) && std::isfinite(curr)) {
                double denominator = std::max(NUMERICAL_STABILITY_THRESHOLD, std::abs(prev));
    ...
    double getSuccessRate(size_t window) const {
        ...
        auto start = decision_outcomes.end() - std::min(window, decision_outcomes.size());
        return std::count(start, decision_outcomes.end(), true) /
               static_cast<double>(std::distance(start, decision_outcomes.end()));
    }

    // Utility function to handle potential NaN/Inf values
    static double safeValue(double value) { return std::isfinite(value) ? value : 0.0; }
...
public:
    ...
    // Debug helpers
    double getCurrentVariance() const { return obj_history.size() >= 2 ? getRecentVariance(obj_history.size()) : 0.0; }

    double getCurrentImprovement() const {
        return obj_history.size() >= 2 ? getRecentImprovement(obj_history.size()) : 0.0;
    }
};
