...
class PDLP {
public:
    PDLP(const Eigen::MatrixXd& A, const Eigen::VectorXd& b, const Eigen::VectorXd& c,
         const Eigen::VectorXd& lower_bounds, const Eigen::VectorXd& upper_bounds)
        : A(A), b(b), c(c), lower_bounds(lower_bounds), upper_bounds(upper_bounds) 
    {
        x = Eigen::VectorXd::Zero(A.cols());
        y = Eigen::VectorXd::Zero(A.rows());
    }

    Eigen::VectorXd proximal_operator(const Eigen::VectorXd& x, const Eigen::VectorXd& lower, const Eigen::VectorXd& upper) {
        return x.cwiseMax(lower).cwiseMin(upper);
    }

    Eigen::VectorXd dual_residuals(const Eigen::VectorXd& x, const Eigen::VectorXd& y) {
        return A.transpose() * y + c;
    }

    double corrected_dual_objective(const Eigen::VectorXd& x, const Eigen::VectorXd& y) {
        return (b.transpose() * y - c.transpose() * x).value();
    }

    std::pair<bool, bool> check_infeasibility(const Eigen::VectorXd& y) {
        bool primal_inf = (A.transpose() * y).isZero(1e-6) && (b.transpose() * y < 0);
    ...
    Eigen::VectorXd iterate(double tau, double sigma, int max_iter = 1000, double tol = 1e-5) {
        for (int i = 0; i < max_iter; ++i) {
            Eigen::VectorXd x_new = proximal_operator(x - tau * (A.transpose() * y + c), lower_bounds, upper_bounds);
