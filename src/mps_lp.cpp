#include "../third_party/cupdlp/mps_lp.h"
#include "../third_party/cupdlp/cupdlp_linalg.h"
...
void print_script_usage() {
    ...
    printf("\n");
    printf("  -savesol <bool> : whether to write solution to .json solution file\n");
    printf("\n");
    // printf(
...
void freealldata(int *Aeqp, int *Aeqi, double *Aeqx, int *Aineqp, int *Aineqi, double *Aineqx, int *colUbIdx,
                 double *colUbElem, double *rhs, double *cost, double *x, double *s, double *t, double *sx, double *ss,
                 double *st, double *y, double *lower, double *upper) {
    if (Aeqp) { cupdlp_free(Aeqp); }

    if (Aeqi) { cupdlp_free(Aeqi); }

    if (Aeqx) { cupdlp_free(Aeqx); }

    if (Aineqp) { cupdlp_free(Aineqp); }

    if (Aineqi) { cupdlp_free(Aineqi); }

    if (Aineqx) { cupdlp_free(Aineqx); }

    if (colUbIdx) { cupdlp_free(colUbIdx); }

    if (colUbElem) { cupdlp_free(colUbElem); }

    if (rhs) { cupdlp_free(rhs); }

    if (cost) { cupdlp_free(cost); }
    ...
    if (sx) { cupdlp_free(sx); }

    if (ss) { cupdlp_free(ss); }

    if (st) { cupdlp_free(st); }

    if (y) { cupdlp_free(y); }

    if (lower) { cupdlp_free(lower); }

    if (upper) { cupdlp_free(upper); }
}
...
cupdlp_retcode data_alloc(CUPDLPdata *data, cupdlp_int nRows, cupdlp_int nCols, void *matrix,
                          CUPDLP_MATRIX_FORMAT src_matrix_format, CUPDLP_MATRIX_FORMAT dst_matrix_format) {
    cupdlp_retcode retcode = RETCODE_OK;
    ...
    case DENSE:
        ...
        CUPDLP_CALL(dense_alloc_matrix(data->dense_matrix, nRows, nCols, matrix, src_matrix_format));
        break;
    case CSR:
        CUPDLP_CALL(csr_create(&data->csr_matrix));
        CUPDLP_CALL(csr_alloc_matrix(data->csr_matrix, nRows, nCols, matrix, src_matrix_format));
        break;
    case CSC:
        CUPDLP_CALL(csc_create(&data->csc_matrix));
        CUPDLP_CALL(csc_alloc_matrix(data->csc_matrix, nRows, nCols, matrix, src_matrix_format));
        break;
    case CSR_CSC:
        CUPDLP_CALL(csc_create(&data->csc_matrix));
        CUPDLP_CALL(csc_alloc_matrix(data->csc_matrix, nRows, nCols, matrix, src_matrix_format));
...
cupdlp_retcode problem_alloc(CUPDLPproblem *prob, cupdlp_int nRows, cupdlp_int nCols, cupdlp_int nEqs,
                             cupdlp_float *cost, cupdlp_float offset, cupdlp_float sense_origin, void *matrix,
                             CUPDLP_MATRIX_FORMAT src_matrix_format, CUPDLP_MATRIX_FORMAT dst_matrix_format,
                             cupdlp_float *rhs, cupdlp_float *lower, cupdlp_float *upper,
                             cupdlp_float *alloc_matrix_time, cupdlp_float *copy_vec_time) {
    cupdlp_retcode retcode = RETCODE_OK;
    prob->nRows            = nRows;
    prob->nCols            = nCols;
    prob->nEqs             = nEqs;
    ...
    CUPDLP_CALL(data_alloc(prob->data, nRows, nCols, matrix, src_matrix_format, dst_matrix_format));
