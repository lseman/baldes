...
void IPSolver::convert_to_standard_form(const Eigen::SparseMatrix<double> &A, const Eigen::VectorXd &b,
    ...
    if (b.size() != n || c.size() != m) { throw std::invalid_argument("Size mismatch in inputs"); }
    ...
    int slack_counter = 0;
    for (int i = 0; i < n; ++i) {
        if (sense(i) == 0) { triplets.emplace_back(i, m + n_free + slack_counter++, 1.0); }
    }
...
void IPSolver::solve_newton_system(
    ...
    const Eigen::VectorXd &delta_y, const Eigen::VectorXd &delta_z, double delta_0, const Eigen::VectorXd &iter_x,
    const Eigen::VectorXd &iter_lambda, const Eigen::VectorXd &iter_w, const Eigen::VectorXd &iter_s,
    const Eigen::VectorXd &iter_v, double iter_tau, double iter_kappa, const Eigen::VectorXd &xi_p,
    const Eigen::VectorXd &xi_u, const Eigen::VectorXd &xi_d, double xi_g, const Eigen::VectorXd &xi_xs,
    ...
    Delta_tau = (xi_g + xi_tau_kappa * inv_tau + c_dot_Delta_x - b_dot_Delta_lambda + ubv_dot_Delta_w) / delta_0;
...
void IPSolver::run_optimization(ModelData &model, const double tol) {
    ...
    Eigen::VectorXi ubi = Eigen::Map<Eigen::VectorXi>(indices.data(), static_cast<int>(indices.size()));
    Eigen::VectorXd ubv = Eigen::Map<Eigen::VectorXd>(ubv_std.data(), static_cast<int>(ubv_std.size()));
    ...
    for (int k = 0; k < max_iter; ++k) {
        ...
        while (ncor <= 2 && alpha < 0.9995) {
            ...
            solve_newton_system(Delta_x_c, Delta_lambda_c, Delta_w_c, Delta_s_c, Delta_v_c, Delta_tau_c, Delta_kappa_c,
                                ...
                                tau, kappa, zero_rp, zero_ru, zero_rd, 0, -t_xs, -t_vw, -t0);
...
int IPSolver::update_linear_solver(SparseSolver &ls, const Eigen::VectorXd &theta, const Eigen::VectorXd &regP,
    ...
    for (int i = 0; i < ls.n; ++i) { ls.S.coeffRef(i, i) = -theta[i] - regP[i]; }
    for (int i = 0; i < ls.m; ++i) { ls.S.coeffRef(ls.n + i, ls.n + i) = regD[i]; }
