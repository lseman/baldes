 ...
 * @brief Python bindings for the BucketGraph, VRPNode, Label, PSTEPDuals, and
 * BucketOptions classes using pybind11.
 *
 * This file defines the Python module `baldes` and exposes the following
 * classes:
 * - VRPNode: Represents a node in the VRP (Vehicle Routing Problem).
 * - Label: Represents a label used in the labeling algorithm.
 * - BucketGraph: Represents the graph structure used in the bucket-based VRP
 * solver.
 * - PSTEPDuals: Represents dual values used in the PSTEP algorithm.
 * - BucketOptions: Represents configuration options for the bucket-based VRP
 * solver.
 *
 * Each class is exposed with its constructors, member variables, and member
 * functions. Conditional compilation is used to expose additional fields and
 * methods based on defined macros.
 */
...
PYBIND11_MODULE(pybaldes, m) {
    ...
    // Bind ParamType for both double and int
    py::class_<ParamType<double>>(m, "DoubleParam")
        .def(py::init<double, size_t>(), py::arg("value"), py::arg("index") = 0)
        .def_readonly("value", &ParamType<double>::value)
        .def_readonly("index", &ParamType<double>::index);

    py::class_<ParamType<int>>(m, "IntParam")
        .def(py::init<int, size_t>(), py::arg("value"), py::arg("index") = 0)
        .def_readonly("value", &ParamType<int>::value)
